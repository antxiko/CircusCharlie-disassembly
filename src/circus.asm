...
lee_de_vram:
	...
	exx			;401f
	ei			;4020   ; la lectura tambien entro con `di`, y se cierra igual
	ret			;4021
...
gancho_de_interrupcion:
	...
	call suena_un_cuadro		;4030   ; primero el sonido, que no puede esperar
	ld hl,0e005h		;4033   ; el cerrojo de reentrada: cuenta cuantas interrupciones se han solapado
	ld a,(hl)			;4036   ; A se queda con el valor de ANTES, que es el que decide
...
L_404B:
	ei			;404b
	reti		;404c   ; sale de la interrupcion con las interrupciones ya abiertas
...
despacha_por_tabla:
	...
	inc hl			;4054
	ld d,(hl)			;4055
	ex de,hl			;4056   ; el destino, a HL, que es el unico par con el que se puede saltar
...
intercambia_bloques:
	...
	ld a,c			;405b   ; el que estaba guardado va al otro lado
	ld (de),a			;405c
	inc hl			;405d
	inc de			;405e   ; y el siguiente par de bytes
...
L_4068:
	...
	inc hl			;4069
	ld d,(hl)			;406a   ; la palabra del destino, byte bajo primero
	inc hl			;406b
L_406C:
	ld a,(hl)			;406c   ; el siguiente byte del guion
	inc hl			;406d
	ld b,a			;406e   ; B es la copia con la que se miran los dos codigos de control sin perder A
...
init:
	di			;407b
	im 1		;407c   ; modo 1: la interrupcion entra por 0x0038 y la BIOS la encamina a H.KEYI
	...
	ld (0fd9bh),hl		;4086   ; y detras del `jp`, la direccion del gancho
	ld sp,0e500h		;4089   ; la pila, por encima de las variables
	ld hl,0e000h		;408c   ; y las variables, a cero de 0xE000 a 0xE3FF
	ld de,0e001h		;408f
	ld bc,003ffh		;4092
	ld (hl),000h		;4095   ; el clasico ldir sobre si mismo: se pone UN cero y el resto lo arrastra
	ldir		;4097
	ld a,001h		;4099   ; el cerrojo, tomado durante el arranque
	ld (0e005h),a		;409b   ; nadie va a atender un cuadro mientras se monta el hardware
	call arranca_el_hardware		;409e   ; prepara el VDP y la pantalla
	xor a			;40a1   ; el hardware ya esta, se puede soltar
...
reparte_la_escena:
	...
	inc (hl)			;40ae   ; un cuadro mas; no se pone nunca a cero, da la vuelta sola
	ld a,(0e002h)		;40af   ; el bit 6 de (0xE002) decide cual de los dos remates se apila
	and 040h		;40b2   ; aisla el bit 6
...
L_40E9:
	...
	call vuelca_los_registros_del_vdp		;40ef   ; y vuelca los ocho registros del VDP
	jr L_4114		;40f2
...
L_4105:
	...
	dec (hl)			;4108   ; un cuadro menos del plazo
	...
	xor a			;4110   ; la bandera de 0xE00A se apaga al borrar el rotulo
...
L_4117:
	...
	ret c			;411a
L_411B:
	xor a			;411b   ; escena 0, la primera de la partida
	jp L_41BC		;411c
...
L_4129:
	...
	ret p			;412c   ; `ret p`: con el bit 7 puesto todavia queda cuenta pendiente
	...
	ldir		;4144   ; los nueve, de la plantilla de 0x43B4 al bloque del jugador
	ld (0e059h),bc		;4146   ; BC vale cero al salir del ldir, y se aprovecha para dejar (0xE059) a cero
	jr $+104		;414a   ; sigue en 0x41B2, detras de la tabla de datos: por eso el destino no sale con nombre
...
pasa_el_guion_de_la_atraccion:
	...
	dec (hl)			;4184   ; un cuadro menos de este paso
	...
	ld (de),a			;418d   ; el paso en curso, a la vista de todo el cartucho
	ld a,b			;418e
...
L_41BC:
	ld (0e004h),a		;41bc   ; el plazo, donde lo mira la escena siguiente
...
L_41C4:
	...
	or a			;41ca   ; cero es "la partida sigue"
	jr z,L_41D3		;41cb
...
L_41F0:
	...
	ld (0e004h),a		;420e
	ld a,021h		;4211   ; y 33 en (0xE00C), el plazo con el que arranca la cuenta de continuar
	ld (0e00ch),a		;4213
	ret			;4216
L_4217:
...
L_421D:
	ld a,009h		;421d   ; escena 9, el compas de la partida
	jr L_424C		;421f
...
L_4235:
	...
	dec (hl)			;4238   ; un cuadro menos del cartel de relevo
	ret nz			;4239
	ld a,(0e080h)		;423a   ; el estado del otro jugador
	or a			;423d   ; cero es "el otro no tiene partida"
	jr nz,escena_13_con_plazo_de_32		;423e
	ld hl,0e002h		;4240
	ld a,(hl)			;4243
	and 0bfh		;4244   ; apaga el bit 6 de (0xE002)
	ld (hl),a			;4246
	jp L_41A3		;4247   ; sigue el mismo jugador, por la escena 0 de la partida
escena_13_con_plazo_de_32:
	ld a,00dh		;424a   ; escena 13
L_424C:
	ld (0e000h),a		;424c   ; la escena elegida, que es lo que mira el reparto del cuadro siguiente
...
L_4260:
	...
	ld a,(hl)			;4263
	or a			;4264   ; si ya esta a cero no se baja mas: la espera la remata el plazo de abajo
	jr z,L_4269		;4265
	dec (hl)			;4267   ; una menos y a esperar
	ret			;4268
L_4269:
	ld a,(0e009h)		;4269   ; el paso del guion de la atraccion
	or a			;426c   ; cero quiere decir que el guion de la atraccion se acabo
	jr nz,arranca_al_otro_jugador		;426d
	ld hl,0e004h		;426f
	dec (hl)			;4272   ; el plazo del cartel
	ret nz			;4273
	...
	ld (0e000h),a		;427c
	xor a			;427f   ; plazo cero: el "GAME OVER" no se quita solo
	jp L_41BC		;4280
...
arranca_al_otro_jugador:
	...
	ldir		;428c   ; los seis, de la plantilla al bloque del que entra
	ld a,003h		;428e   ; tres vidas
	ld (0e050h),a		;4290   ; tres vidas para el que entra
	ld a,(0e002h)		;4293   ; el turno
	add a,a			;4296   ; el bit 7 al acarreo
	ld hl,0e049h		;4297   ; el tanteo de un jugador
	jr nc,L_429E		;429a   ; el acarreo -el bit 7- elige cual de los dos tanteos
	ld l,046h		;429c   ; o el del otro, cambiando solo el byte bajo
L_429E:
	xor a			;429e   ; tres bytes de tanteo BCD a cero
	ld (hl),a			;429f   ; tres bytes a cero, que son los seis digitos BCD
	inc hl			;42a0
	ld (hl),a			;42a1
	inc hl			;42a2
	ld (hl),a			;42a3
	...
	ld (0e000h),a		;42a6
	jp L_4217		;42a9   ; y de ahi a mirar si hay que ceder el turno
...
consume_la_bonificacion:
	...
	call suma_puntos		;42b7   ; lo que la bonificacion pierde, el tanteo lo GANA: los mismos dieciseis
	call resta_a_la_bonificacion		;42ba   ; y se repinta
	ld a,(0e003h)		;42bd   ; el contador de cuadros
	and 003h		;42c0   ; uno de cada cuatro
	ret nz			;42c2
	ld a,006h		;42c3   ; el sonido 6, el tictac
	jp pide_un_sonido		;42c5
...
termina_la_fase:
	...
	or a			;42cb   ; mientras (0xE028) no sea cero queda algo en movimiento
	ret nz			;42cc
	ld hl,0e050h		;42cd   ; las vidas
	inc (hl)			;42d0   ; una mas
	inc hl			;42d1
	ld a,(hl)			;42d2   ; (0xE051), el numero de fase
	add a,001h		;42d3   ; la fase siguiente, en BCD
	daa			;42d5
	ld (hl),a			;42d6
	inc hl			;42d7
	inc (hl)			;42d8   ; y el contador de fases dentro de la tanda
	ld a,(hl)			;42d9   ; el contador de fases dentro de la tanda
	cp 005h		;42da   ; cada cinco
	jr nz,L_42ED		;42dc
	xor a			;42de
	ld (hl),a			;42df   ; se reinicia
	ld hl,0e05bh		;42e0
	ld a,(hl)			;42e3   ; el decorado en curso
...
L_42ED:
	...
	ld (0e057h),hl		;42fb
	xor a			;42fe   ; cero para las dos banderas de abajo
	ld (0e00dh),a		;42ff   ; se baja la senal de fin de partida
	ld (0e059h),a		;4302   ; y (0xE059), que cuenta los avisos pendientes
	jp L_4233		;4305
remata_la_fase:
...
L_430E:
	...
	call pide_un_sonido		;4316
	ld a,050h		;4319   ; 80 cuadros de plazo
	ld (0e004h),a		;431b
L_431E:
	call limpia_la_pantalla		;431e   ; limpia
	call carga_los_graficos_del_titulo		;4321   ; y monta el decorado del cartel
	call barre_la_pantalla_entera		;4324   ; y se barre entera de una vez
	ld hl,0e001h		;4327
	inc (hl)			;432a   ; se marca que ya se ha hecho
	ret			;432b
L_432C:
	ld hl,0e004h		;432c   ; el plazo
	dec (hl)			;432f   ; el plazo del cartel de fin de partida
	...
	call donde_va_el_cursor		;4337   ; la celda donde esta el cursor
	ld bc,0001bh		;433a   ; 27 celdas que borrar
	xor a			;433d   ; cero: lo que se escribe es la celda vacia
...
carga_los_graficos_del_titulo:
	...
	call descomprime_en_de		;434d
	ld de,05600h		;4350   ; y la tabla de color
	ld bc,00180h		;4353   ; 0x180 bytes, que es la tabla de color de un tercio de pantalla
...
L_4366:
	...
	call descomprime_en_de		;436b
	pop hl			;436e
	ld bc,00010h		;436f   ; y cada vuelta dieciseis celdas mas alla
	add hl,bc			;4372   ; dieciseis celdas mas alla en cada vuelta
	ex de,hl			;4373
	pop bc			;4374
	djnz L_4366		;4375
	ret			;4377
...
reparte_los_puntos_pendientes:
	...
	push af			;438c   ; la respuesta se guarda para el final, que vuelve a hacer falta
	jr z,L_4394		;438d
	...
	inc bc			;4393
L_4394:
	ld d,h			;4394   ; DE = HL+1: es lo que convierte el `ldir` en un arrastre del cero
	ld e,l			;4395
	inc e			;4396
...
baja_los_dos_contadores:
	...
	dec (hl)			;43c0   ; el contador de cuadros baja aqui, que es la unica cuenta atras del barrido
	inc hl			;43c1
	...
	jr c,L_43CB		;43c7
	xor 01fh		;43c9   ; en media vuelta el indice va al reves, y de ahi sale el vaiven
L_43CB:
	ld e,a			;43cb   ; el indice, convertido en columna
...
saca_los_sprites_de_pantalla:
	...
	xor a			;43e6   ; sale con A a cero y sin acarreo, que es lo que espera quien llama
	ret			;43e7
...
L_43F4:
	...
	ld (0e043h),bc		;440c
	ld (0e044h),bc		;4410   ; las dos escrituras van SOLAPADAS a proposito: entre ellas dejan 99 99 99 en 0xE043..0xE045
	jr L_4477		;4414
...
mira_si_es_record:
	...
	ld b,(hl)			;4439   ; los digitos altos del tanteo
	...
	sbc hl,de		;4442   ; el acarreo llega limpio del `sub` de arriba, asi que la resta es exacta
	jr nc,L_4480		;4444
L_4446:
	ld (0e043h),de		;4446   ; el tanteo nuevo manda
	ld a,b			;444a   ; y el par alto, que ya estaba en B
	ld (0e045h),a		;444b
	jr L_4477		;444e
...
L_446B:
	...
	call pinta_el_numero_de_fase		;446e
	ld hl,0e058h		;4471   ; y la bonificacion
	call pinta_la_bonificacion_y_mira_el_final		;4474
L_4477:
	ld hl,0e045h		;4477   ; el record
	ld de,0380fh		;447a   ; en la fila de arriba, columna 15
	call imprime_tres_bytes_bcd		;447d
L_4480:
	ld a,(0e002h)		;4480
...
pinta_el_tanteo:
	...
	jr nc,imprime_tres_bytes_bcd		;448a
	ld e,025h		;448c   ; la columna del segundo tanteo
	ld hl,0e048h		;448e   ; y la variable del segundo
imprime_tres_bytes_bcd:
	ld b,003h		;4491   ; tres bytes de BCD
	jr imprime_bcd		;4493

; ----------------------------------------------------------------------
; EL NUMERO DE FASE DEL MARCADOR, el "STAGE". Lo que imprime es (0xE051) -un byte, dos digitos BCD-, que es el contador que sube de uno en uno en 0x42D3 al terminar cada fase. NO es la bonificacion: esa son los dos bytes de (0xE057) y la pinta 0x52A0.
; ----------------------------------------------------------------------
pinta_el_numero_de_fase:
	ld de,0381ch		;4495   ; la celda del marcador, fila 0 columna 28
	ld hl,0e051h		;4498   ; (0xE051), el numero de fase en BCD
	ld b,001h		;449b   ; un solo byte, o sea dos digitos
...
imprime_bcd:
	...
	ld c,a			;44a3   ; el digito de abajo, apartado en C hasta que le toque
	pop af			;44a4   ; y se recupera el byte entero
	and 0f0h		;44a5   ; y el de arriba
	rra			;44a7   ; cuatro veces a la derecha
	rra			;44a8
	rra			;44a9
	rra			;44aa
	or 010h		;44ab   ; el mismo desplazamiento para el de arriba
	call escribe_en_vram		;44ad   ; primero el alto
	inc de			;44b0
	ld a,c			;44b1   ; y ahora el que estaba esperando en C
...
; ----------------------------------------------------------------------
; EL PARPADEO DEL "1P" / "2P", y todo el, mascara incluida, sale del contador de cuadros. El `ld hl,(0e002h)` trae de un tiron las banderas en L y el contador en H. La rutina solo hace algo cuando los cinco bits de abajo del contador estan a cero, o sea un cuadro de cada 32; y la mascara con la que se pinta es el BIT 5 DE ESE MISMO CONTADOR: a cero deja C=0x00 -que en el interprete de rotulos es BORRAR- y a uno lo baja a 0xFF, que es pintar. De ahi salen los 32 cuadros encendido y 32 apagado. Quien esta en turno solo elige CUAL de los dos rotulos, con el bit 7 de (0xE002).
; ----------------------------------------------------------------------
parpadea_el_turno:
	ld hl,(0e002h)		;44ba   ; de un tiron: las banderas en L y el contador de cuadros en H
	ld a,01fh		;44bd   ; los cinco bits de abajo
	and h			;44bf
	ret nz			;44c0   ; solo uno de cada 32 cuadros
	ld c,a			;44c1   ; y aqui A vale cero: la mascara 0x00, la de BORRAR
	bit 5,h		;44c2   ; el bit 5 del CONTADOR, no de las banderas: es lo que alterna cada 32 cuadros
	jr z,L_44C7		;44c4
	dec c			;44c6   ; C pasa de 0x00 a 0xFF, o sea de borrar a pintar
L_44C7:
	bit 7,l		;44c7   ; el bit 7 dice de quien es el turno
	ld hl,049b7h		;44c9   ; el rotulo de uno
	jr z,L_44D1		;44cc   ; el turno solo elige cual de los dos rotulos
...
pinta_las_vidas:
	...
	ld c,a			;44da   ; las vidas, en el contador del bucle
	ld b,005h		;44db   ; cinco marcas como maximo
	ld a,00bh		;44dd   ; la marca
L_44DF:
	dec c			;44df   ; una menos
	jp p,L_44E5		;44e0   ; mientras la resta no se pase de cero, marca
...
barre_la_pantalla_entera:
	...
	ld (0e00ah),a		;44ee   ; la cortina se baja desde la fila 0
L_44F1:
	call barre_una_franja		;44f1   ; y se barre hasta que la rutina diga que ha terminado
	jr c,L_44F1		;44f4   ; mientras devuelva acarreo queda franja por barrer
	ret			;44f6
...
barre_una_franja:
	...
	ld a,(hl)			;44fa
	inc (hl)			;44fb   ; y se deja apuntada para la llamada siguiente
	cp 011h		;44fc   ; diecisiete pasadas
	jr nc,L_451C		;44fe
	ld de,03888h		;4500   ; la esquina de arriba
	ld c,a			;4503   ; el paso, apartado: hace falta dos veces
	...
	ld c,a			;450a   ; el patron con el que se pinta esta franja
	ld b,003h		;450b   ; tres filas de golpe
	xor a			;450d   ; la primera de las tres filas va en blanco
L_450E:
	call escribe_en_vram		;450e
	ld a,020h		;4511   ; la de abajo
	call suma_a_a_de		;4513
	ld a,c			;4516   ; y las dos de abajo con el patron, que ademas avanza
	inc c			;4517
	djnz L_450E		;4518
	scf			;451a   ; acarreo: todavia queda
	ret			;451b
L_451C:
	push af			;451c   ; la cuenta se guarda para el `cp` de abajo
	ld hl,049c3h		;451d   ; "PLAY SELECT" con sus cuatro opciones
	call z,pinta_rotulo		;4520   ; el menu se pinta justo en la pasada 17, cuando el barrido acaba
	pop af			;4523
...
L_4531:
	...
	ld a,b			;4534   ; el primero de los dos patrones
	call escribe_en_vram		;4535   ; los dos patrones, uno detras de otro
	ld a,c			;4538
	inc de			;4539   ; y el de al lado
...
donde_va_el_cursor:
	...
	rrca			;4544
	ld e,a			;4545   ; el byte bajo ya es la celda: no hay que sumar nada mas
...
pinta_rotulo_con_marca:
	...
	dec de			;4553   ; la celda de delante del rotulo
...
; ----------------------------------------------------------------------
limpia_la_pantalla:
	call saca_los_sprites_de_pantalla		;4558
...
L_4566:
	...
	ld a,b			;456d   ; BC a cero se mira en dos pasos, que el `dec bc` no toca banderas
	or c			;456e
	jr nz,L_4566		;456f   ; hasta que BC llega a cero
	ei			;4571   ; el relleno entro con `di`; aqui se abre
	ret			;4572
...
copia_vram_a_vram:
	...
	ld a,c			;459a   ; igual aqui: el `dec bc` no levanta el cero
	or b			;459b
...
descomprime_donde_quedo:
	ld a,(hl)			;45d1   ; el byte de control
	and 07fh		;45d2   ; los siete bits de abajo son la cuenta
	ld c,a			;45d4   ; la cuenta, apartada en C
	ld a,(hl)			;45d5   ; y el byte otra vez entero, que su bit 7 es lo que decide la rama
	inc hl			;45d6
...
L_45DE:
	...
	push af			;45e1   ; el resultado del `cp` hace falta dos veces, una por rama
	...
	di			;45e9   ; las dos ramas salen con las interrupciones abiertas; el bucle las quiere cerradas
	jr descomprime_donde_quedo		;45ea
...
prepara_escritura_vram:
	ex af,af'			;45ec   ; A se aparta, que SETWRT lo usa
	ex de,hl			;45ed   ; SETWRT quiere la direccion en HL
	call 00053h		;45ee   ; BIOS SETWRT - Enables VDP to write | SETWRT de la BIOS: el puntero de escritura
	di			;45f1   ; SETWRT vuelve con las interrupciones abiertas, y aqui no interesan
	ex de,hl			;45f2   ; y HL y DE, como estaban
	exx			;45f3
...
copia_a_los_tres_tercios:
	push de			;4608
	call copia_dos_tercios		;4609
	pop de			;460c
	ld hl,00800h		;460d   ; un tercio son 0x800 bytes
	add hl,de			;4610   ; un tercio mas alla
	ex de,hl			;4611
copia_dos_tercios:
	...
	add hl,bc			;461d   ; los DOS punteros suben 0x2000, el que lee y el que escribe
	ex de,hl			;461e
	add hl,bc			;461f
	ex de,hl			;4620
	pop bc			;4621
copia_bloque_guardando:
	push bc			;4622   ; esta puerta guarda los tres pares: quien llama encadena copias sin recargar nada
	push de			;4623
	push hl			;4624
...
L_4634:
	...
	add hl,bc			;4644   ; el `ld c,008h` de arriba es el avance bueno, el que NO retrocede
...
espeja_patrones:
	...
	rlca			;4649
	ld b,a			;464a   ; el par de bits de arriba, apartado
	ld a,c			;464b
	...
	call suma_a_a_hl		;465d   ; y sobre el bufer de entrada
L_4660:
	push bc			;4660
...
L_4665:
	...
	inc de			;466b   ; el byte siguiente de la salida
	inc hl			;466c
	dec c			;466d   ; ocho bytes armados, ocho columnas del dibujo
	jr nz,L_4663		;466e
	ld a,0f0h		;4670   ; +0xF0 y `dec d`: retrocede dieciseis, o sea la fila de arriba
	call suma_a_a_de		;4672
	dec d			;4675
	pop bc			;4676   ; y la altura, de vuelta
	djnz L_4660		;4677
	pop bc			;4679
	ret			;467a
...
prepara_la_figura:
	...
	add a,a			;467c   ; por ocho, que es lo que ocupa un patron
	add a,a			;467d
	add a,a			;467e
...
saca_una_columna_del_bufer:
	ld a,(ix+000h)		;46a0   ; un byte del bufer
	ld (hl),a			;46a3   ; y al hueco de paso, seguidos
	add ix,de		;46a4   ; de ocho en ocho: lo que se saca es una COLUMNA, no una fila
	inc hl			;46a6
	djnz saca_una_columna_del_bufer		;46a7
	pop ix		;46a9
	pop bc			;46ab   ; la cuenta de patrones, de vuelta
	ld a,003h		;46ac   ; tres pasadas
	push iy		;46ae
gira_tres_veces:
	ex af,af'			;46b0   ; la cuenta de pasadas se aparta en A'
	push bc			;46b1
	call desplaza_el_bufer		;46b2   ; una pasada del giro
	pop bc			;46b5
	push bc			;46b6
	push iy		;46b7
	ld hl,0e378h		;46b9   ; y lo desplazado vuelve del hueco de paso
L_46BC:
	ld a,(hl)			;46bc
	ld (iy+000h),a		;46bd   ; cada byte a su altura del bufer
	inc hl			;46c0
	add iy,de		;46c1   ; otra vez de ocho en ocho
	djnz L_46BC		;46c3
	pop iy		;46c5
	ld a,(0e37fh)		;46c7   ; la altura guardada en 0x467F
	ld c,a			;46ca
	add iy,bc		;46cb   ; y una altura mas: cada pasada deja su copia por encima de la anterior
	pop bc			;46cd
	ex af,af'			;46ce   ; la cuenta de pasadas, de vuelta
	dec a			;46cf   ; tres pasadas
	jr nz,gira_tres_veces		;46d0
	pop iy		;46d2
	inc ix		;46d4   ; el patron siguiente, una columna a la derecha en los dos punteros
	inc iy		;46d6
	dec c			;46d8
	jr nz,L_4697		;46d9
	ret			;46db
...
desplaza_el_bufer:
	push bc			;46dc
	call rota_el_bufer_un_bit		;46dd   ; dos llamadas encadenadas, o sea DOS bits de desplazamiento
	pop bc			;46e0
rota_el_bufer_un_bit:
	ld hl,0e378h		;46e1   ; el hueco de paso
	ld a,b			;46e4
	call suma_a_a_hl		;46e5   ; HL al final del hueco, que el arrastre va de atras hacia delante
	push hl			;46e8
	xor a			;46e9   ; y el acarreo limpio para la primera rotacion
L_46EA:
	...
	dec hl			;46f1   ; el byte de delante
...
; ----------------------------------------------------------------------
arranca_el_hardware:
	ld a,0b8h		;46f4   ; 0xB8 en el mezclador: los tres tonos abiertos, los tres ruidos cerrados y el puerto A como entrada, que es por donde se leen los mandos
	call escribe_el_mezclador_del_psg		;46f6
	ld a,094h		;46f9   ; el sonido de arranque
	call pide_un_sonido		;46fb
	ld de,00000h		;46fe   ; desde el byte 0 de la VRAM
...
L_4713:
	ld hl,0e038h		;4713   ; y se vuelcan desde la COPIA en RAM, no desde la ROM
...
L_4718:
	...
	inc hl			;471c
	inc c			;471d   ; el registro siguiente: van los ocho seguidos, de R0 a R7
	dec d			;471e
	jr nz,L_4718		;471f
	ret			;4721
...
codigo_huerfano_472A:
	...
	jr $-26		;472d   ; el destino existe -es 0x4713, el vuelco de los registros del VDP-, pero nadie llega hasta aqui para tomarlo
...
lee_el_mando:
	...
	jr z,lee_un_puerto_del_psg		;4736   ; con el bit 7 a cero, el primer puerto
...
lee_los_mandos:
	...
	ld c,(hl)			;4753   ; la de antes, que estaba ahi desde el cuadro pasado
	ld (hl),a			;4754
	dec hl			;4755
	ld (hl),c			;4756   ; y queda en (0xE008), que es contra lo que se miran los flancos
	ret			;4757
...
lee_el_teclado:
	...
	rrca			;475e   ; una rotacion
	and 020h		;475f   ; y se queda con su bit
	ld e,a			;4761   ; guardado en E
	ld a,008h		;4762   ; y otra fila
	call 00141h		;4764   ; BIOS SNSMAT - Returns the value of the specified line from the keyboard matrix
	cpl			;4767   ; pulsado es cero aqui tambien
	rrca			;4768   ; dos rotaciones
	rrca			;4769
	ld b,a			;476a   ; el resto, en B
	and 004h		;476b   ; la tecla de esta fila
	or e			;476d   ; juntada con la de antes
	ld c,a			;476e   ; en C
	ld a,b			;476f   ; la otra fila, recolocada a su sitio
	rrca			;4770   ; dos rotaciones mas
	rrca			;4771
	ld b,a			;4772   ; otra vez en B
	and 018h		;4773   ; las dos teclas de esta fila
	or c			;4775   ; juntadas
	ld c,a			;4776   ; en C
	ld a,b			;4777   ; y lo ultimo
	rrca			;4778   ; una rotacion
	and 003h		;4779   ; y las dos que quedan
	or c			;477b   ; juntadas: las seis teclas caben en un byte
	ret			;477c
...
atiende_el_menu:
	...
	call lee_un_puerto_del_psg		;477f
	ld d,a			;4782   ; guardado
	ld e,0cfh		;4783   ; y el segundo
	call lee_un_puerto_del_psg		;4785
	or d			;4788   ; se juntan
	ld d,a			;4789   ; los dos juntos
	call lee_el_teclado		;478a   ; mas el teclado
	or d			;478d   ; y el teclado encima: se juega con cualquiera de los tres
	ld hl,0e040h		;478e   ; lo de ahora en (0xE040)
	ld c,(hl)			;4791   ; lo de antes
	ld (hl),a			;4792   ; y lo de antes justo detras
	inc hl			;4793   ; y la anterior, en (0xE041)
	ld (hl),c			;4794
	ld b,a			;4795   ; en B
	...
	ld a,(hl)			;47b5   ; el modo que le toca a esa opcion
	...
	ld (0e000h),hl		;47bc   ; las DOS a la vez: escena 8 en (0xE000) y cero en (0xE001), que es la bandera de la escena de fin
	ret			;47bf
salta_a_la_escena_5:
	ld (hl),a			;47c0   ; a la escena 5
...
mueve_el_cursor:
	push bc			;47c4
	call pinta_el_cursor		;47c5   ; borra el cursor de donde estaba
	pop af			;47c8   ; los bits recien pulsados vuelven en A
	ld hl,0e042h		;47c9   ; la opcion elegida
	ld b,(hl)			;47cc
	rra			;47cd   ; el bit 0 es "arriba"
	jr nc,L_47D1		;47ce
...
L_47D5:
	ld a,b			;47d5
	and 003h		;47d6   ; cuatro opciones, y el `and` las hace dar la vuelta sin un solo `cp`
	ld (hl),a			;47d8
	ret			;47d9
...
prepara_la_cortinilla:
	...
	ld (0e00ah),a		;4b4b   ; el contador de pasos de la cortinilla
	ld hl,00000h		;4b4e   ; y se empieza por arriba del todo
	ld (0e00eh),hl		;4b51
	ld hl,04ba6h		;4b54   ; los patrones de la cortina
	ld de,06208h		;4b57   ; 0x6208: los dos bits de arriba son la orden de escritura, la celda es la 0x208 de los patrones
	call descomprime_en_de		;4b5a
	ld de,00208h		;4b5d   ; y la misma celda de la tabla de color, que va 0x2000 mas abajo
...
baja_la_cortinilla:
	...
	ex de,hl			;4b75
	or a			;4b76   ; limpia el acarreo para el `sbc` del reflejo
	ld hl,03aaah		;4b77   ; el punto de reflejo
	sbc hl,de		;4b7a   ; restando sale la franja de abajo
	ex de,hl			;4b7c   ; la franja de abajo, al puntero de escritura
	...
	ld b,c			;4b8a   ; y la tercera, de doce: la cuenta venia guardada en C
	call pinta_franja_correlativa		;4b8b
	xor a			;4b8e   ; lo que queda de la fila, en blanco
...
L_4B98:
	...
	inc de			;4b9b   ; la celda de al lado
	inc a			;4b9c   ; el siguiente, uno mas
	djnz L_4B98		;4b9d
	pop de			;4b9f   ; DE vuelve al principio de la franja
	ld hl,00020h		;4ba0   ; y sale apuntando una fila mas abajo, que es lo que encadena las llamadas
	add hl,de			;4ba3
	ex de,hl			;4ba4
	ret			;4ba5
...
monta_la_fase:
	call monta_el_decorado_de_la_fase		;4c39   ; el decorado
	call limpia_el_estado_de_la_fase		;4c3c   ; y los que se mueven
	ret			;4c3f
elige_el_remate_del_cuadro:
	cp 007h		;4c40   ; por debajo de siete
	jr nc,$+103		;4c42   ; salta a 0x4CA9, el vuelco de los sprites: el destino cae detras y por eso no sale con nombre
	ld a,(0e052h)		;4c44   ; y segun el tipo de fase
	cp 003h		;4c47
	jr z,$+108		;4c49   ; a 0x4CB5, el cuadro de la fase de los recogibles
	jr $+94		;4c4b   ; y en los demas casos tambien a 0x4CA9
mascara_38:
...
limita_los_mandos:
	...
	jr nc,remata_el_cuadro_del_jugador		;4c5d
	ld a,034h		;4c5f   ; 0x34: una direccion menos que la otra mascara
L_4C61:
	push af			;4c61   ; la mascara se guarda: hay que aplicarla en dos sitios
	ld hl,0e009h		;4c62   ; la lectura de este cuadro
	and (hl)			;4c65   ; recortada
	ld (hl),a			;4c66
	pop af			;4c67   ; y se recupera para el segundo
	ld hl,0e138h		;4c68   ; y la copia de (0xE138), igual
	and (hl)			;4c6b
	ld (hl),a			;4c6c
remata_el_cuadro_del_jugador:
	ld a,(0e134h)		;4c6d   ; el estado del jugador
	cp 006h		;4c70   ; de 6 en adelante es un final
	push af			;4c72   ; la respuesta se guarda hasta despues del despacho
	jr nc,L_4C7B		;4c73
	ld a,(0e052h)		;4c75
	or a			;4c78
	jr z,L_4C7E		;4c79
L_4C7B:
	call despacha_el_estado_del_jugador		;4c7b   ; se despacha
L_4C7E:
	pop af			;4c7e
	jr nc,elige_el_remate_del_cuadro		;4c7f
...
despacha_por_tipo_de_fase:
	...
	push af			;4c84
	and 003h		;4c85   ; con los dos bits de abajo a cero no se cuenta paso
	jr z,L_4C8C		;4c87
	call cuenta_el_paso		;4c89
L_4C8C:
	call baja_la_bonificacion_cada_ocho		;4c8c
	pop af			;4c8f   ; y el tipo entero de vuelta, que es el indice de la tabla
...
; ----------------------------------------------------------------------
L_4C9D:
	call mueve_el_decorado		;4c9d
	call mueve_los_tres_moviles		;4ca0
L_4CA3:
	call pinta_el_borde_del_decorado		;4ca3
	call desplaza_el_decorado		;4ca6
L_4CA9:
	...
	jp vuelca_bloque_a_vram		;4cb2   ; y de una tirada
L_4CB5:
	call mueve_los_seis_recogibles		;4cb5
	jr L_4CA3		;4cb8
L_4CBA:
	call mueve_los_seis_recogibles		;4cba
	call mueve_el_suelto		;4cbd
	jr L_4CA3		;4cc0
...
fase_4_el_cuadro_entero:
	...
	call avanza_la_segunda_figura		;4ccb
	call repinta_lo_que_sujeta		;4cce
	call pasa_el_relevo_a_la_segunda		;4cd1
	jr L_4CA9		;4cd4   ; y remata por el vuelco de sprites, como todas
L_4CD6:
	call mueve_lo_que_se_empuja		;4cd6
	call saca_la_siguiente_de_la_cola		;4cd9
	call mira_la_cola_contra_el_jugador		;4cdc
	call pinta_la_cola		;4cdf
	jr L_4CA9		;4ce2
...
L_4D2F:
	...
	call pide_un_sonido		;4d4d
	jr L_4D59		;4d50
L_4D52:
	ld a,007h		;4d52   ; siete bytes por objeto
	call suma_a_a_hl		;4d54   ; siete bytes mas alla: el objeto siguiente
	djnz L_4D2F		;4d57   ; y los tres objetos de la lista
L_4D59:
	ld de,0e156h		;4d59   ; el temporizador de 0xE156
	call caja_estrecha		;4d5c
	ld a,003h		;4d5f   ; el sonido queda pedido solo si la caja dio acarreo
	call c,apunta_por_indice		;4d61
	...
	or a			;4d6e   ; la fase 0 es la unica con meta
	call z,mira_la_caja_del_final		;4d6f   ; la fase 0 tiene su propia comprobacion
...
fase_2_el_tope_de_altura:
	...
	ld e,000h		;4d86   ; el tramo bajo no sube nada
	ld a,(0e218h)		;4d88   ; el tramo
	add a,a			;4d8b   ; por dos, con el acarreo al bit 0
	jr nc,L_4D8F		;4d8c
	inc a			;4d8e
L_4D8F:
	cp 004h		;4d8f   ; por debajo de cuatro se queda con lo que salio del tramo
	ld d,a			;4d91
	jr c,L_4DFC		;4d92
...
toca_un_objeto:
	...
	push af			;4dc2   ; el indice se guarda: hace falta en las dos listas
	ld hl,0e180h		;4dc3
	call suma_a_a_hl		;4dc6
	ld (hl),000h		;4dc9   ; se apaga en su lista
	pop af			;4dcb   ; el mismo indice, ahora para la lista de posiciones
	ld hl,0e170h		;4dcc
...
L_4DE4:
	...
	inc hl			;4de6   ; veinticuatro celdas seguidas del bufer
	djnz L_4DE4		;4de7
	ld a,(0e218h)		;4de9
	cp 001h		;4dec
	ld de,00200h		;4dee
...
aterriza:
	...
	cp (hl)			;4e18   ; el suelo, contra donde esta
	jr nc,cierra_el_cuadro_del_jugador		;4e19   ; si ya esta mas abajo, se deja
	ld (hl),a			;4e1b
	inc hl			;4e1c   ; dos bytes mas alla vive el estado
	inc hl			;4e1d
...
L_4E29:
	...
	ld a,03eh		;4e35
	sub (hl)			;4e37   ; contra la otra coordenada del aro
	...
	call pide_un_sonido		;4e48
	ld a,(0e009h)		;4e4b   ; la lectura del mando se congela en (0xE138)
	ld (0e138h),a		;4e4e   ; y la copia queda congelada mientras dure el gesto
	ld hl,051bah		;4e51   ; la rampa del gesto
	jp arranca_gesto_con_cero		;4e54
se_queda_donde_estaba:
	ld hl,0e130h		;4e57   ; la altura del jugador
	ld a,085h		;4e5a   ; el tope
	cp (hl)			;4e5c   ; contra donde esta ahora
	jr nc,cierra_el_cuadro_del_jugador		;4e5d   ; si ya esta por debajo, se deja
	ld (hl),a			;4e5f   ; y por encima del tope, se clava
	ld e,000h		;4e60   ; el byte bajo a cero: el premio va en centenas
	ld a,(0e218h)		;4e62   ; el tramo, que es lo que vale el premio
	add a,a			;4e65   ; por dos
	ld d,a			;4e66   ; el tramo por dos, en el byte alto: 200, 400, 600... segun lo alto que se llegue
L_4E67:
	call suma_puntos		;4e67   ; y se suman los puntos
	xor a			;4e6a   ; estado 0: a correr otra vez
	ld (0e134h),a		;4e6b   ; estado 0, y con el
	ld (0e140h),a		;4e6e   ; el contador de la animacion
	ld (0e218h),a		;4e71   ; y el tramo, los tres a cero de una tacada
cierra_el_cuadro_del_jugador:
	call mira_la_caja_del_gesto		;4e74   ; primero lo que pueda estar haciendo el gesto
	jp nc,mira_los_choques		;4e77   ; si no hay nada especial, el cuadro normal
	ld a,002h		;4e7a   ; estado 2
	ld (0e134h),a		;4e7c
	ld hl,0e1e0h		;4e7f   ; los dos bits de (0xE1E0) que dicen como va la figura
	res 7,(hl)		;4e82   ; se apaga el bit 7 y se enciende el 6 de (0xE1E0)
	set 6,(hl)		;4e84
	ld hl,0e1c0h		;4e86
	ld a,(hl)			;4e89   ; el byte del dibujo
	xor 010h		;4e8a   ; y se le da la vuelta al bit 4 de (0xE1C0): el dibujo alterna
	ld (hl),a			;4e8c
	ld de,00500h		;4e8d   ; quinientos puntos
	jp suma_puntos		;4e90   ; y se suman de camino a la salida: `jp`, no `call`
mira_la_caja_del_final:
	ld hl,0e14dh		;4e93
	ld a,(0e130h)		;4e96   ; la posicion del jugador
	add a,020h		;4e99   ; centrada sobre la figura
	sub (hl)			;4e9b   ; contra la de la meta
	cp 005h		;4e9c   ; cinco pixeles de margen en esta
	ret nc			;4e9e
	dec hl			;4e9f   ; el byte de al lado es la otra
	ld a,03ch		;4ea0   ; la otra coordenada, contra 0x3C
	sub (hl)			;4ea2
	inc hl			;4ea3
	cp 030h		;4ea4   ; y 0x30 de margen: la caja es ancha aqui
	ret			;4ea6   ; sale con el acarreo puesto si cae dentro de la caja
...
estado_0_corriendo:
	...
	jr nz,L_4EBC		;4eaf   ; con direccion pulsada, se avanza
	ld a,(0e052h)		;4eb1   ; en la fase 4 se avanza siempre
	cp 004h		;4eb4   ; la fase 4 avanza aunque no se pulse nada
	jr z,L_4EBC		;4eb6
	ld a,001h		;4eb8   ; y quieto, el dibujo 1
	jr L_4EC8		;4eba
L_4EBC:
	ld bc,000c0h		;4ebc   ; 0xC0 por cuadro
	add hl,bc			;4ebf   ; 0xC0 de 0x100 es poco mas de medio pixel por cuadro
	ld (0e137h),hl		;4ec0   ; la posicion, de vuelta a su sitio
	ld a,h			;4ec3   ; los bits 2 y 3 del byte alto
	and 00ch		;4ec4   ; los bits 2 y 3
	rrca			;4ec6   ; bajados a su sitio: el paso de la animacion
	rrca			;4ec7
L_4EC8:
	ld (0e132h),a		;4ec8   ; y ese es el dibujo que toca, sin contador de animacion aparte
	ld a,(0e052h)		;4ecb   ; el tipo de fase
	cp 003h		;4ece   ; la fase 3 es la unica que se cae
	jr nz,L_4EE5		;4ed0
	call mira_si_pisa_el_borde		;4ed2   ; en la fase 3 hay que mirar si se cae
	jr nc,L_4EE5		;4ed5
	ld hl,0e139h		;4ed7
	set 3,(hl)		;4eda   ; se marca la caida
	ld hl,0e18ch		;4edc
	inc (hl)			;4edf   ; y el dibujo pasa al siguiente
	ld a,006h		;4ee0   ; estado 6
	ld (0e134h),a		;4ee2   ; estado 6, el de la caida
L_4EE5:
	call se_acaba_de_pulsar_boton		;4ee5   ; mira si se ha pulsado salto
	jr z,cierra_el_cuadro_del_jugador		;4ee8   ; si no, cuadro normal
	xor a			;4eea   ; el contador de 0xE272, a cero al empezar el salto
	ld (0e272h),a		;4eeb
	ld hl,051e9h		;4eee   ; la rampa de la caida
	call arranca_gesto_con_cero		;4ef1   ; el gesto arranca por el paso 0
	ld a,001h		;4ef4   ; estado 1: saltando
	ld (0e134h),a		;4ef6   ; estado 1: en el aire
	ld a,(0e009h)		;4ef9   ; la lectura del mando se congela para todo el salto
	ld (0e138h),a		;4efc   ; la lectura se congela
	ld (0e139h),a		;4eff   ; en las DOS copias, que el salto mira las dos
	ld a,(0e052h)		;4f02   ; el tipo de fase decide el sonido del salto
	cp 001h		;4f05   ; y la fase 1 tiene el suyo
	ld a,002h		;4f07   ; uno
	jr z,L_4F0D		;4f09
L_4F0B:
	ld a,003h		;4f0b   ; y el otro
L_4F0D:
	jp pide_un_sonido		;4f0d
...
; ----------------------------------------------------------------------
estado_1_saltando:
	call se_acaba_de_pulsar_boton		;4f10   ; se ha soltado el boton?
	jr z,estado_2_la_voltereta		;4f13   ; con el boton suelto, la voltereta
	ld hl,0e1e0h		;4f15   ; la bandera de "esta en el aire"
	set 7,(hl)		;4f18   ; se marca que esta en el aire
	ld hl,0e1c0h		;4f1a
	ld a,(hl)			;4f1d
	push af			;4f1e   ; el paso se guarda antes de tocarle el bit del dibujo
	xor 010h		;4f1f   ; el dibujo alterna en cada cuadro
	ld (hl),a			;4f21
	pop af			;4f22   ; y se recupera para mirarle los cinco bits de abajo
...
L_4F35:
	ld b,001h		;4f35   ; con B a uno el gesto arranca por el paso 1
	ld hl,051e6h		;4f37   ; y su rampa de alturas
	jr L_4F59		;4f3a
L_4F3C:
	ld b,000h		;4f3c   ; y aqui por el 0
	cp 007h		;4f3e   ; el primer tramo se parte otra vez en tres
	jr nc,L_4F49		;4f40
	ld a,0f0h		;4f42   ; dieciseis pixeles
	ld hl,051deh		;4f44   ; cada uno con SU rampa de alturas
	jr L_4F59		;4f47
L_4F49:
	cp 009h		;4f49   ; el segundo corte
	jr nc,L_4F54		;4f4b
...
L_4F59:
	...
	ld a,b			;4f5c   ; el paso por el que arranca el gesto
	call arranca_gesto_con_a		;4f5d
	ld a,003h		;4f60   ; estado 3
	ld (0e134h),a		;4f62
	jr L_4F0B		;4f65   ; y de ahi a pedir el sonido del salto
...
estado_2_la_voltereta:
	...
	or a			;4f6a
	ret z			;4f6b   ; sin voltereta en marcha no hay nada que hacer
...
L_4F79:
	...
	push af			;4f7c   ; el paso entero se guarda: la altura lo quiere con cuatro bits y el dibujo con cinco
	and 00fh		;4f7d   ; los cuatro bits de abajo: diez alturas
	call suma_a_a_hl		;4f7f   ; indexa la curva
	ld a,(hl)			;4f82   ; y esa es la altura del cuadro
	ld (0e130h),a		;4f83   ; y esa es la altura de este cuadro
	pop af			;4f86
	...
	sub 005h		;4f8e   ; menos cinco, que las poses de ida y las de vuelta no caen en el mismo sitio
	and 01fh		;4f90
L_4F92:
	cp 007h		;4f92   ; y de ahi salen tres poses
	jr c,L_4F9A		;4f94   ; de siete en adelante
	ld a,003h		;4f96   ; la tercera pose
	jr L_4FA2		;4f98
L_4F9A:
	cp 004h		;4f9a
	ld a,000h		;4f9c
	...
	ld a,001h		;4fa0
L_4FA2:
	ld (0e132h),a		;4fa2   ; y el dibujo elegido, donde lo lee el dibujante
...
mira_los_choques:
	...
	ld a,(0e052h)		;4fac
	or a			;4faf
	jr z,jugador_alcanzado		;4fb0   ; y en la fase 0 tampoco se comprueba
	...
	jr nz,L_4FC1		;4fbd
	ld b,000h		;4fbf   ; en la fase 2 no se desplaza nada
L_4FC1:
	ld a,d			;4fc1
	sub (hl)			;4fc2   ; la distancia en una coordenada
	add a,b			;4fc3   ; con el desplazamiento encima
	cp 038h		;4fc4   ; cincuenta y seis pixeles de caja
	jr nc,L_4FD5		;4fc6
	inc hl			;4fc8
	ld a,e			;4fc9   ; la otra coordenada
	sub (hl)			;4fca   ; la distancia en la otra
	add a,030h		;4fcb
	cp 004h		;4fcd   ; si esta muy cerca, choque
	jr c,llega_a_la_meta		;4fcf   ; muy cerca es haber llegado
	cp 038h		;4fd1   ; y dentro de la caja, un choque
	jr c,se_ha_caido		;4fd3
L_4FD5:
	call mira_la_caja_de_5238		;4fd5   ; y si no, las demas comprobaciones
	jr c,L_4FED		;4fd8
	call mira_los_tres_moviles		;4fda   ; las tres figuras que se mueven
	jr c,L_4FED		;4fdd
	call mira_los_moviles_de_cerca		;4fdf   ; las mismas, de cerca
	call mira_los_seis_contra_el_jugador		;4fe2   ; los seis recogibles
	jr c,se_ha_caido		;4fe5
	call mira_todo_lo_recogible		;4fe7   ; y lo que queda por recoger
jugador_alcanzado:
	jp dibuja_al_jugador		;4fea   ; sin nada de eso, el cuadro se cierra dibujando al jugador
L_4FED:
	ld de,05231h		;4fed   ; la lista de siete valores
	call reparte_la_lista_de_siete		;4ff0
...
estado_por_la_tabla_5000:
	...
	or a			;5003   ; no se cambia de estado hasta que se calle
	ret nz			;5004
	ld a,008h		;5005   ; estado 8
	ld (0e134h),a		;5007
	ld a,08fh		;500a   ; y el sonido del final
	jp pide_un_sonido		;500c
L_500F:
	ld a,(0e028h)		;500f   ; espera a que pare todo
	or a			;5012   ; ni hasta que pare todo lo que se mueve
	ret nz			;5013
...
se_ha_caido:
	...
	cp 001h		;501b   ; la fase 1 se va por el estado 9
	jr z,L_4FF6		;501d
	cp 004h		;501f   ; y la 4 tiene sonido propio
	ld a,006h		;5021   ; estado 6
	ld (0e134h),a		;5023
	jr nz,L_4FFB		;5026   ; las demas se van con el de siempre
	ld a,007h		;5028   ; el sonido de la caida
	jp pide_un_sonido		;502a
baja_dos_y_aterriza:
	ld hl,0e130h		;502d
	inc (hl)			;5030   ; dos pixeles hacia abajo
	inc (hl)			;5031   ; dos pixeles, uno por instruccion
	jp aterriza		;5032
...
L_50A4:
	...
	add a,a			;50a8
	sub d			;50a9
	ld de,05116h		;50aa   ; la tabla de patrones de sprite de esa pose
L_50AD:
	call suma_a_a_de		;50ad
	ld hl,0e0c4h		;50b0   ; el bufer de atributos en RAM
	call mete_dos_sprites		;50b3   ; el primer sprite
	ld a,010h		;50b6   ; dieciseis pixeles mas abajo: el segundo
	add a,b			;50b8   ; mas la fila
	ld b,a			;50b9   ; guardada
	call mete_dos_sprites		;50ba   ; y el segundo
	ld a,(0e052h)		;50bd   ; el tipo de fase
	cp 004h		;50c0
	jr z,dibuja_al_jugador_de_la_fase_4		;50c2   ; la fase 4 va aparte
	cp 001h		;50c4
	ret nz			;50c6   ; y las demas se quedan en dos
	ld a,b			;50c7   ; en la fase 1 el jugador son CUATRO sprites, no dos
	add a,010h		;50c8   ; dieciseis mas abajo
	ld b,a			;50ca   ; guardada
	ld a,c			;50cb   ; la columna
	sub 008h		;50cc   ; ocho a la izquierda
	ld c,a			;50ce   ; guardada
	call mete_un_sprite_con_patron		;50cf   ; el tercero
	ld a,c			;50d2   ; la columna otra vez
	add a,010h		;50d3   ; y dieciseis a la derecha
	ld c,a			;50d5   ; guardada
	jr mete_dos_sprites		;50d6

; ----------------------------------------------------------------------
; EL JUGADOR DE LA FASE 4, que lleva TRES sprites y una animacion propia. El paso sale de los bits 2 y 3 del contador de cuadros -o sea, cambia cada cuatro cuadros- y se multiplica por tres, que es lo que ocupa cada paso en la tabla de 0x5155.
; ----------------------------------------------------------------------
dibuja_al_jugador_de_la_fase_4:
	ld a,(0e003h)		;50d8   ; el contador de cuadros
	rra			;50db   ; dos giros: uno de cada cuatro
	rra			;50dc
	and 003h		;50dd   ; cuatro pasos
	ld d,a			;50df
	add a,a			;50e0   ; por tres
	add a,d			;50e1
	ld de,05155h		;50e2   ; la tabla de doce bytes, cuatro poses de tres
	call suma_a_a_de		;50e5
	ld a,(0e134h)		;50e8   ; el estado del jugador
	cp 006h		;50eb
	jr c,L_50F5		;50ed
	ld a,(0e0d5h)		;50ef
	add a,008h		;50f2
	ld c,a			;50f4
L_50F5:
	ld b,0a6h		;50f5   ; la fila de arriba
	ld a,c			;50f7
	sub 005h		;50f8   ; cinco pixeles a la izquierda
	ld c,a			;50fa
	call mete_un_sprite_con_patron		;50fb
	ld a,c			;50fe
	add a,010h		;50ff   ; y dieciseis a la derecha
	ld c,a			;5101
	call mete_un_sprite_con_patron		;5102
	ld b,096h		;5105   ; la fila de abajo
	jr mete_un_sprite_con_patron		;5107

; ----------------------------------------------------------------------
; METER UN SPRITE EN EL BUFER: cuatro bytes -fila, columna, patron y color-, con el patron sacado de la tabla que apunta DE. La puerta de 0x5109 mete DOS de golpe, que es lo que ocupa una figura de 16x16 partida en dos.
; ----------------------------------------------------------------------
mete_dos_sprites:
	call mete_un_sprite_con_patron		;5109
mete_un_sprite_con_patron:
	ld (hl),b			;510c   ; la fila
	inc hl			;510d
	ld (hl),c			;510e   ; la columna
	inc hl			;510f
	ld a,(de)			;5110   ; el patron, de la tabla
	inc de			;5111
	ld (hl),a			;5112
	inc hl			;5113
	inc hl			;5114   ; y el color se deja como estaba
	ret			;5115
...
; ----------------------------------------------------------------------
; ARRANCAR UN GESTO. HL trae la rampa que hay que recorrer y A su sentido; queda apuntada en (0xE200), la senal en (0xE202) y el paso a cero. Las dos puertas solo se diferencian en A, y el `jr $+4` de 0x5162 salta POR ENCIMA del `ld a,001h`.
; ----------------------------------------------------------------------
arranca_gesto_con_cero:
	xor a			;5161   ; sentido 0
	jr arranca_gesto_con_a		;5162
L_5164:
	ld a,001h		;5164   ; y por aqui se entraria con sentido 1
arranca_gesto_con_a:
	ld (0e200h),hl		;5166   ; la rampa que hay que recorrer
	ld (0e202h),a		;5169   ; la senal de "hay gesto en marcha"
	xor a			;516c
	ld (0e205h),a		;516d   ; y el paso, a cero
	ret			;5170

; ----------------------------------------------------------------------
; AVANZAR UN GESTO UN PASO. El sentido de (0xE202) decide si el paso sube o baja, y a partir del octavo cambia la pose de 1 a 4.
; ----------------------------------------------------------------------
avanza_el_gesto:
	ld a,(0e202h)		;5171   ; el sentido
	ld b,a			;5174
	ld hl,0e205h		;5175
	bit 0,b		;5178   ; el bit 0
	jr nz,L_517F		;517a
	inc (hl)			;517c   ; hacia delante
	jr L_5180		;517d
L_517F:
	dec (hl)			;517f   ; o hacia atras
L_5180:
	ld a,(hl)			;5180
	cp 008h		;5181   ; a partir del paso 8
	ld a,001h		;5183
	jr c,L_5189		;5185
	ld a,004h		;5187   ; la otra pose
L_5189:
	ld (0e132h),a		;5189
	ld hl,0e130h		;518c   ; la posicion del jugador
	ld de,(0e200h)		;518f   ; por donde va la rampa
	bit 0,b		;5193
	ld a,(de)			;5195   ; el desplazamiento de este paso
	jr nz,L_519A		;5196
	neg		;5198   ; al reves si el gesto va marcha atras
L_519A:
	add a,(hl)			;519a
	ld (hl),a			;519b   ; y se aplica
	ex de,hl			;519c
	bit 0,b		;519d
	jr nz,L_51A4		;519f
	inc hl			;51a1   ; la rampa avanza
	jr L_51A5		;51a2
L_51A4:
	dec hl			;51a4   ; o retrocede
L_51A5:
	ld a,(hl)			;51a5
	inc a			;51a6   ; un 0xFF en la rampa es el final
	jr nz,L_51B3		;51a7
	dec hl			;51a9
	dec hl			;51aa
	inc a			;51ab
	ld (0e202h),a		;51ac   ; se baja la senal: se acabo el gesto
L_51AF:
	ld (0e200h),hl		;51af
	ret			;51b2
L_51B3:
...
; ----------------------------------------------------------------------
; MIRAR SI EL JUGADOR PISA EL BORDE de alguno de los SEIS tramos de 0xE170. Basta con cinco pixeles de margen; si ninguno casa, se sale con la bandera limpia.
; ----------------------------------------------------------------------
mira_si_pisa_el_borde:
	ld b,006h		;5200   ; seis tramos
	ld hl,0e170h		;5202
L_5205:
	ld a,04fh		;5205   ; la referencia
	sub (hl)			;5207
	jr c,L_520D		;5208
	cp 005h		;520a   ; cinco pixeles de margen
	ret c			;520c
L_520D:
	inc hl			;520d   ; dos bytes por tramo
	inc hl			;520e
	djnz L_5205		;520f
	or a			;5211
	ret			;5212
...
; ----------------------------------------------------------------------
; EL FLANCO DEL BOTON. Invierte la lectura ANTERIOR y la cruza con la de ahora: solo queda a uno el boton que estaba suelto y ahora esta pulsado. Es lo que hace que mantener apretado no cuente como pulsar dos veces.
; ----------------------------------------------------------------------
se_acaba_de_pulsar_boton:
	ld hl,0e008h		;5213
	ld a,(hl)			;5216   ; lo de antes
	cpl			;5217   ; invertido
	and 030h		;5218   ; los dos botones
	inc hl			;521a
	and (hl)			;521b   ; cruzado con lo de ahora: el flanco
	ret			;521c
...
; ----------------------------------------------------------------------
; OTRA CAJA DE CONTACTO, esta de 0x10 por 0x26: mas estrecha en una coordenada que en la otra, que es como se afinan los saltos.
; ----------------------------------------------------------------------
mira_la_caja_de_5238:
	ld hl,0e157h		;5238
	ld a,(0e130h)		;523b
	add a,030h		;523e
	sub (hl)			;5240
	cp 010h		;5241   ; dieciseis pixeles
	ret nc			;5243
	dec hl			;5244
	ld a,04dh		;5245
	sub (hl)			;5247
	cp 026h		;5248   ; y treinta y ocho en la otra
	ret			;524a

; ----------------------------------------------------------------------
; LA CAJA CONTRA LOS TRES MOVILES de 0xE150. El desplazamiento cambia segun el bit 7 del movil -0x18 o 0x10-, o sea que un movil "mira" a un lado o a otro y su caja se corre con el.
; ----------------------------------------------------------------------
mira_los_tres_moviles:
	ld b,003h		;524b   ; tres moviles
	ld hl,0e150h		;524d
	ld de,0e160h		;5250
L_5253:
	ld a,038h		;5253   ; la referencia
	sub (hl)			;5255
	inc hl			;5256
	cp 016h		;5257   ; veintidos pixeles
	jr nc,L_526C		;5259
	ld a,(de)			;525b
	bit 7,a		;525c   ; el bit 7 dice hacia donde mira
	ld a,(0e130h)		;525e
	ld c,018h		;5261   ; veinticuatro de desplazamiento
	jr z,L_5267		;5263
	ld c,010h		;5265   ; o dieciseis
L_5267:
	sub c			;5267
	sub (hl)			;5268
	cp 026h		;5269   ; y treinta y ocho de caja
	ret c			;526b
L_526C:
	inc hl			;526c   ; dos bytes por movil
	inc de			;526d
	inc de			;526e
	djnz L_5253		;526f
	ret			;5271
mira_la_caja_del_gesto:
	ld a,(0e202h)		;5272   ; solo si hay gesto en marcha
	or a			;5275
	ret z			;5276
	ld hl,0e1d6h		;5277
	ld a,(0e130h)		;527a
	add a,014h		;527d
	sub (hl)			;527f
	cp 020h		;5280   ; treinta y dos pixeles
	ret nc			;5282
	inc hl			;5283
	ld a,04fh		;5284
	sub (hl)			;5286
	cp 030h		;5287   ; y cuarenta y ocho en la otra
	ret			;5289
baja_la_bonificacion_cada_ocho:
	ld a,(0e003h)		;528a   ; el contador de cuadros
	and 007h		;528d   ; uno de cada ocho
	ret nz			;528f

; ----------------------------------------------------------------------
; RESTAR A LA BONIFICACION, en BCD. Baja de DIECISEIS en dieciseis con `sub 010h / daa`, y si hay acarreo se le quita uno al byte de arriba. Aqui es donde se gasta el 5000 con el que empieza la fase.
; ----------------------------------------------------------------------
resta_a_la_bonificacion:
	ld hl,0e057h		;5290
	ld a,(hl)			;5293
	sub 010h		;5294   ; dieciseis menos
	daa			;5296   ; en BCD
	ld (hl),a			;5297
	inc hl			;5298
	jr nc,pinta_la_bonificacion_y_mira_el_final		;5299
	ld a,(hl)			;529b
	sub 001h		;529c   ; el byte alto, uno menos
	daa			;529e
	ld (hl),a			;529f
pinta_la_bonificacion_y_mira_el_final:
	ld de,03832h		;52a0   ; la celda de la bonificacion
	ld b,002h		;52a3   ; dos bytes, cuatro digitos
	call imprime_bcd		;52a5
	ld a,(0e000h)		;52a8   ; la escena en curso
	cp 00bh		;52ab   ; solo la escena 11 mira el final
	ret nz			;52ad
	inc hl			;52ae
	inc hl			;52af
	ld a,(hl)			;52b0
	cp 010h		;52b1   ; 0x10 en el byte alto: queda mil y pico
	dec hl			;52b3
	jr z,avisa_de_que_queda_poco		;52b4
	or (hl)			;52b6
	ret nz			;52b7
	jp L_4FF6		;52b8   ; y si llego a cero de verdad, el jugador se ha quedado sin tiempo

; ----------------------------------------------------------------------
; EL AVISO DE QUE SE ACABA EL TIEMPO, y no es un sonido fijo: el tipo de fase elige entre tres -0x8D, 0x8B y 0x89-, y ademas cambia el canal (B). El `ex af,af'` guarda el numero mientras se coloca el otro dato.
; ----------------------------------------------------------------------
avisa_de_que_queda_poco:
	ld a,(hl)			;52bb
	or a			;52bc
	ret nz			;52bd
	ld a,(0e052h)		;52be   ; el tipo de fase
	or a			;52c1
	ld c,a			;52c2
	ld b,003h		;52c3
	ld a,08dh		;52c5   ; el aviso de las fases de tipo 0
	jr z,L_52D3		;52c7
	ld b,005h		;52c9
	ld a,08bh		;52cb   ; el de las de tipo par
	bit 1,c		;52cd
	jr nz,L_52D3		;52cf
	ld a,089h		;52d1   ; y el de las demas
L_52D3:
	ex af,af'			;52d3
	ld a,b			;52d4
	ex af,af'			;52d5
	jp pide_un_sonido		;52d6

; ----------------------------------------------------------------------
; LA SEGUNDA CAJA CONTRA LOS TRES MOVILES, esta mucho mas estrecha: DOS pixeles en una coordenada. Y como en la otra, el margen en la segunda -0x3F o 0x2F- depende de hacia donde mire el movil.
; ----------------------------------------------------------------------
mira_los_moviles_de_cerca:
	ld b,003h		;52d9   ; tres moviles
	ld hl,0e150h		;52db
	ld de,0e160h		;52de
L_52E1:
	ld a,02ch		;52e1
	sub (hl)			;52e3
	inc hl			;52e4
	cp 002h		;52e5   ; dos pixeles nada mas
	jr nc,L_52FB		;52e7
	ld a,(de)			;52e9
	bit 7,a		;52ea   ; hacia donde mira
	ld a,(0e130h)		;52ec
	ld c,03fh		;52ef   ; sesenta y tres
	jr z,L_52F5		;52f1
	ld c,02fh		;52f3   ; o cuarenta y siete
L_52F5:
	add a,004h		;52f5
	sub (hl)			;52f7
	cp c			;52f8
	jr c,apunta_objeto_recogido		;52f9
L_52FB:
	inc hl			;52fb
	inc de			;52fc
	inc de			;52fd
...
; ----------------------------------------------------------------------
; APUNTAR UN OBJETO RECOGIDO. Del contador del bucle sale el indice al reves -`ld a,003h / sub b`, o `006h / sub b` por la otra puerta-, se apaga esa casilla de 0xE210 y se sube el contador de 0xE218. Si la casilla ya estaba a cero se sale sin contar nada, que es lo que impide cobrar dos veces el mismo objeto.
; ----------------------------------------------------------------------
apunta_objeto_recogido:
	ld a,003h		;5301   ; tres menos lo que queda: el indice
	sub b			;5303
apunta_por_indice:
	ld hl,0e210h		;5304   ; la lista de casillas
	call suma_a_a_hl		;5307
	ld a,(hl)			;530a
	or a			;530b   ; ya estaba cogido: no se cuenta
	ret z			;530c
	xor a			;530d
	ld (hl),a			;530e   ; se apaga
L_530F:
	ld hl,0e218h		;530f
	inc (hl)			;5312   ; y uno mas al contador
	ret			;5313
apunta_objeto_de_seis:
	ld a,006h		;5314   ; la misma cuenta, pero sobre seis
	sub b			;5316
	jr apunta_por_indice		;5317
apunta_el_objeto_de_e217:
	ld hl,0e217h		;5319
	ld a,(hl)			;531c   ; la casilla suelta de 0xE217
	or a			;531d
	ret z			;531e
	xor a			;531f
	ld (hl),a			;5320
	inc hl			;5321
	set 7,(hl)		;5322   ; y se marca el bit 7 en la de al lado
	jr L_530F		;5324

; ----------------------------------------------------------------------
; EL BARRIDO DE LO QUE SE PUEDE RECOGER: los SEIS de 0xE170 y, ademas, el suelto de 0xE1B0. Cada uno se mide con la misma caja estrecha.
; ----------------------------------------------------------------------
mira_todo_lo_recogible:
	ld b,006h		;5326   ; seis
	ld de,0e170h		;5328
L_532B:
	call caja_estrecha		;532b
	jr c,apunta_objeto_de_seis		;532e
	inc de			;5330
	inc de			;5331   ; dos bytes cada uno
	djnz L_532B		;5332
	ld de,0e1b0h		;5334   ; y el septimo, aparte
	call caja_estrecha		;5337
	jr c,apunta_el_objeto_de_e217		;533a
	ret			;533c

; ----------------------------------------------------------------------
; LA CAJA MAS ESTRECHA DEL CARTUCHO: DOS pixeles. El desplazamiento es 0x38 en general y 0x20 en las fases de tipo 3, que es lo que la corre a su sitio en esa fase.
; ----------------------------------------------------------------------
caja_estrecha:
	ld a,(0e052h)		;533d   ; el tipo de fase
	cp 003h		;5340
	ld a,(de)			;5342
	ld c,038h		;5343   ; cincuenta y seis
	jr nz,L_5349		;5345
	ld c,020h		;5347   ; o treinta y dos en la fase 3
L_5349:
	sub c			;5349
	cp 002h		;534a   ; dos pixeles de margen
	ret			;534c

; ----------------------------------------------------------------------
; QUE DIRECCION SE ESTA PULSANDO, devuelta como -1, 0 o 1. Si el jugador esta en el estado 0 vale la lectura de este cuadro; si no, la congelada de (0xE138). El `ccf / sbc a,a` convierte el acarreo en 0x00 o 0xFF de una vez.
; ----------------------------------------------------------------------
que_direccion_se_pulsa:
	ld a,(0e134h)		;534d   ; el estado del jugador
	or a			;5350
	ld a,(0e009h)		;5351   ; corriendo: lo que se pulsa ahora
	jr z,L_5359		;5354
	ld a,(0e138h)		;5356   ; y si no, lo que se pulsaba al empezar el gesto
L_5359:
	and 00ch		;5359   ; los dos bits de direccion
	ret z			;535b   ; nada pulsado
	cp 005h		;535c
	ccf			;535e
	sbc a,a			;535f   ; 0xFF o 0x00 segun el acarreo
	ret nz			;5360
	inc a			;5361   ; y +1: queda -1, 0 o 1
	ret			;5362
dibuja_el_tramo:
	ld a,0c3h		;5363   ; el patron de relleno
	ld (0e0f8h),a		;5365
	ld bc,00402h		;5368   ; cuatro filas por dos columnas
	ex de,hl			;536b
	jp pinta_bloque_en_su_sitio		;536c

; ----------------------------------------------------------------------
; EL DECORADO QUE SE MUEVE. Suma -0x50 a la posicion y, si el byte alto pasa de 5, no hay nada que hacer. Es el bucle que va sacando por la izquierda lo que ya se ha pasado.
; ----------------------------------------------------------------------
mueve_el_decorado:
	ld hl,(0e055h)		;536f   ; la posicion del decorado
	ld de,0ffb0h		;5372   ; ochenta hacia atras
	add hl,de			;5375
	ld a,h			;5376
	cp 005h		;5377   ; fuera de la pantalla
	ret nc			;5379
	inc l			;537a
	ld h,0a8h		;537b   ; la fila del decorado
	ld a,l			;537d
	or a			;537e
	jr nz,mueve_el_tramo_de_arriba		;537f
	call que_direccion_se_pulsa		;5381   ; que direccion se pulsa
	dec a			;5384
	ld (0e143h),a		;5385   ; y se guarda para el resto del cuadro
mueve_el_tramo_de_arriba:
	ld a,(0e143h)		;5388   ; lo que hay apuntado
	or a			;538b
	jr z,dibuja_el_tramo		;538c
	bit 7,l		;538e   ; el bit 7 de la posicion
	jr z,L_5397		;5390
	ld a,002h		;5392   ; y entonces la casilla de 0xE213
	ld (0e213h),a		;5394
L_5397:
	ld a,l			;5397
	add a,002h		;5398   ; dos mas
	cp 004h		;539a   ; por debajo de cuatro no se pinta
	jr c,dibuja_el_tramo		;539c
	ld a,(0e003h)		;539e   ; el contador de cuadros
	and 004h		;53a1   ; el bit 2: el dibujo cambia cada cuatro cuadros
	add a,0a0h		;53a3   ; y de ahi sale el patron, 0xA0 o 0xA4
	ld b,a			;53a5
	ld de,09808h		;53a6
	ld a,l			;53a9
	add a,002h		;53aa
	push hl			;53ac
	ld hl,0e0f8h		;53ad
	call mete_un_sprite		;53b0
	pop de			;53b3
	ld (0e156h),de		;53b4   ; la posicion queda apuntada en (0xE156)
	ld hl,071b1h		;53b8   ; la tabla de cuatro rectangulos de 2x4
	ld bc,00204h		;53bb

; ----------------------------------------------------------------------
; PINTAR UN RECTANGULO ELEGIDO POR LA COLUMNA. La tabla trae CUATRO punteros y `ld a,e / and 006h` se queda con dos bits de la columna: cuatro variantes del mismo dibujo, una por cada posicion posible dentro de la celda. Es el apano para que algo que se mueve a media celda no se vea saltar.
; ----------------------------------------------------------------------
elige_rectangulo_y_pinta:
	ld a,e			;53be   ; y se elige uno por la columna
	and 006h		;53bf   ; dos bits de la columna
	call suma_a_a_hl		;53c1
	ld a,(hl)			;53c4   ; y de ahi el rectangulo que toca
	inc hl			;53c5
	ld h,(hl)			;53c6
	ld l,a			;53c7

; ----------------------------------------------------------------------
; PINTAR UN RECTANGULO de B filas por C columnas, leyendo los indices de corrido. El `ld a,e / and 01fh / jr z` corta la fila al llegar al borde derecho de la pantalla, de modo que un dibujo a medio salir no se derrama en la fila de abajo.
; ----------------------------------------------------------------------
pinta_rectangulo:
	call posicion_a_celda		;53c8
	di			;53cb
	set 6,d		;53cc   ; el bit 6: la marca de escritura del VDP
L_53CE:
	push hl			;53ce
	push bc			;53cf
	push de			;53d0
...
L_53D4:
	ld a,(hl)			;53d4   ; un indice de patron
	exx			;53d5
	out (c),a		;53d6
	exx			;53d8
	inc hl			;53d9
	inc de			;53da
	ld a,e			;53db   ; la columna
	and 01fh		;53dc   ; final de fila: se corta
	jr z,L_53E3		;53de
	dec c			;53e0   ; y si no, la celda siguiente
	jr nz,L_53D4		;53e1
L_53E3:
	pop de			;53e3
	ld a,020h		;53e4   ; la fila de abajo, 32 celdas
	call suma_a_a_de		;53e6
	pop bc			;53e9
	pop hl			;53ea
	ld a,c			;53eb   ; el ancho, para saltar a la fila siguiente del dibujo
...
; ----------------------------------------------------------------------
; DE POSICION A CELDA DE PANTALLA. Divide las dos coordenadas entre ocho de una vez, encadenando los `rra` de D con los `rr e` de E, y le pega el 0x38 de la tabla de nombres. Tres bits de cada una, que es justo el tamano de un patron.
; ----------------------------------------------------------------------
posicion_a_celda:
	ld a,d			;53f3
	rra			;53f4   ; cuatro giros
	rra			;53f5
	rra			;53f6
	rra			;53f7
	rr e		;53f8   ; arrastrando el acarreo a la otra coordenada
	rra			;53fa
	rr e		;53fb
	rra			;53fd
	rr e		;53fe
	and 003h		;5400   ; lo que queda de la fila
	add a,038h		;5402   ; mas la pagina de la tabla de nombres
	ld d,a			;5404   ; y la fila, al byte alto
	ret			;5405
direccion_pulsada_cruda:
	ld a,(0e134h)		;5406   ; el estado del jugador
	ld de,0e009h		;5409   ; corriendo: la lectura de ahora
	or a			;540c   ; cero es "corriendo"
	jr z,L_5412		;540d
	ld de,0e138h		;540f   ; y si no, la congelada
L_5412:
	ld a,(de)			;5412   ; la lectura que toque
	and 00ch		;5413   ; los dos bits de direccion
	ret			;5415

; ----------------------------------------------------------------------
; EL BUCLE DE LOS TRES MOVILES. Cada uno tiene su byte de estado en 0xE160, su posicion en 0xE150 y su hueco de sprite en 0xE100; segun el estado se va por un sitio o por otro.
; ----------------------------------------------------------------------
mueve_los_tres_moviles:
	ld ix,0e160h		;5416   ; el estado de cada movil
	ld de,0e100h		;541a   ; su hueco en el bufer de sprites
	ld hl,0e150h		;541d   ; y su posicion
	ld b,003h		;5420   ; tres
L_5422:
	push bc			;5422   ; el contador se guarda: las dos ramas lo gastan
	ld a,(ix+000h)		;5423   ; el estado de este movil
	or a			;5426   ; estado cero: no esta activo
	push hl			;5427   ; la posicion y la respuesta, guardadas
	push af			;5428
	call nz,mueve_el_movil_activo		;5429   ; activo: se mueve
	pop af			;542c
	call z,arranca_movil_parado		;542d   ; y parado: se mira si le toca arrancar
	pop hl			;5430
	inc hl			;5431   ; dos bytes de posicion
	inc hl			;5432
	inc ix		;5433   ; y dos de estado
	inc ix		;5435
	pop bc			;5437
	djnz L_5422		;5438
	ret			;543a
arranca_movil_activo:
	call libera_un_hueco_de_sprite		;543b   ; el que ya estaba suelta UN hueco
	jr L_5443		;543e
arranca_movil_parado:
	call libera_dos_huecos		;5440   ; y el parado, los DOS
L_5443:
	ld (ix-010h),0ffh		;5443   ; se marca el hueco de sprite como libre
	push de			;5447   ; el hueco de sprite, guardado
	call posicion_en_el_decorado		;5448   ; donde cae en el decorado
	jr nc,L_5470		;544b   ; fuera del decorado no hay nada que colocar
	ld a,b			;544d
	push ix		;544e
	pop bc			;5450
	ld hl,00002h		;5451   ; dos bytes mas alla del estado
	add hl,bc			;5454
	dec a			;5455   ; el primero de los tres va por otro lado
	jr nz,coloca_un_movil_del_decorado		;5456
	ld a,l			;5458
	and 0f0h		;5459   ; redondeado a la casilla de abajo
	ld l,a			;545b
coloca_un_movil_del_decorado:
	ld a,(hl)			;545c   ; la casilla del decorado
	or a			;545d   ; vacia
	jr z,L_5470		;545e
	inc hl			;5460   ; el byte de al lado
	ld d,(hl)			;5461   ; lo que hay en ella
	ld a,0efh		;5462   ; 0xEF hacia atras, o sea diecisiete menos con el `dec h` de despues
	call suma_a_a_hl		;5464
	dec h			;5467
	ld a,c			;5468
	rra			;5469   ; tres bits de la coordenada
	and 007h		;546a
	ld e,a			;546c
	call pone_un_movil_en_su_sitio		;546d
L_5470:
	pop de			;5470
	ret			;5471

; ----------------------------------------------------------------------
; DONDE CAE ALGO DENTRO DEL DECORADO: la posicion del decorado (0xE055) menos 0x140. Es la conversion entre "donde va el mundo" y "donde se ve".
; ----------------------------------------------------------------------
posicion_en_el_decorado:
	ld de,0fec0h		;5472   ; el desplazamiento fijo
	ld hl,(0e055h)		;5475   ; la posicion del decorado
	add hl,de			;5478   ; la posicion, corrida
	ret			;5479

; ----------------------------------------------------------------------
; COLOCAR UN MOVIL, si su hueco esta libre y la distancia es menor de TRES. El margen es asi de estrecho a proposito: el movil solo aparece cuando su casilla del decorado pasa justo por su sitio.
; ----------------------------------------------------------------------
pone_un_movil_en_su_sitio:
	ld a,(bc)			;547a   ; el hueco ocupado?
	or a			;547b   ; ocupado: no se coloca nada
	ret nz			;547c
	ld a,d			;547d
	sub (hl)			;547e   ; la distancia
	ret c			;547f   ; detras de la casilla, tampoco
	cp 003h		;5480   ; tres pixeles de margen
	ret nc			;5482
L_5483:
	push bc			;5483   ; el hueco se guarda
	ld hl,0e059h		;5484   ; el contador de apariciones
	inc (hl)			;5487   ; uno mas
	ld a,(hl)			;5488
	inc hl			;5489
	inc (hl)			;548a   ; y el de la tanda tambien
	and 00fh		;548b   ; cada dieciseis
	ld a,(hl)			;548d   ; el de la tanda, ya subido
	jr nz,L_549A		;548e
	ld b,00ch		;5490   ; doce menos
	cp 020h		;5492   ; salvo si venia de 0x20
	jr nz,L_5498		;5494
	ld b,010h		;5496   ; o dieciseis si venia de 0x20
L_5498:
	sub b			;5498   ; y se baja de golpe: es lo que hace que la tanda vuelva a empezar
	ld (hl),a			;5499
L_549A:
	pop bc			;549a
...
L_54B1:
	...
	ld a,(hl)			;54b5   ; el primer byte al estado del movil
	ld (bc),a			;54b6   ; al estado del movil
	inc hl			;54b7
	inc bc			;54b8   ; y el byte de al lado
	ld a,(hl)			;54b9   ; y el segundo detras
	ld (bc),a			;54ba
	ld hl,0e210h		;54bb   ; la lista de marcas
	ld a,e			;54be
	call suma_a_a_hl		;54bf
	ld (hl),002h		;54c2   ; y el movil queda marcado con un 2
	ret			;54c4
...
; ----------------------------------------------------------------------
; MOVER UN MOVIL QUE YA ESTA EN MARCHA. Su avance depende de lo que este pulsando el jugador Y del contador de cuadros: `rra / sbc a,a` convierte el bit 0 del contador en 0 o -1, y eso se suma al sentido. O sea que el movil se mueve a un paso y medio por cuadro, no a uno entero.
; ----------------------------------------------------------------------
mueve_el_movil_activo:
	call que_direccion_se_pulsa		;5505   ; el sentido que pulsa el jugador
	ld b,a			;5508   ; el sentido, apartado
	ld a,(0e003h)		;5509   ; el contador de cuadros
	ld c,a			;550c   ; el contador, apartado: abajo hace falta entero
	rra			;550d   ; su bit 0
	sbc a,a			;550e   ; convertido en 0 o -1
	add a,b			;550f   ; sumado al sentido
	jp z,quita_el_movil		;5510   ; si sale cero, no se mueve
	add a,(hl)			;5513   ; y se aplica a su posicion
	ld (hl),a			;5514   ; la posicion nueva
	cp 002h		;5515   ; con menos de dos no hay nada que repintar
	jr c,suma_los_tres_moviles		;5517
	push hl			;5519   ; la posicion y el hueco, guardados
	push de			;551a
	call mete_el_movil_en_los_sprites		;551b   ; el movil entra en los sprites
	pop de			;551e
	pop hl			;551f
	push hl			;5520
	call quita_el_movil		;5521   ; y se borra de donde estaba
	pop hl			;5524
	push de			;5525
	bit 7,(ix+000h)		;5526   ; el bit 7 del movil dice cual de las dos listas usa
	ld de,06feah		;552a   ; una, con seis por diez
	ld bc,0060ah		;552d   ; seis de ancho por diez de alto
	jr z,pinta_el_movil		;5530
	ld de,07026h		;5532   ; y la otra, con ocho
	ld c,008h		;5535   ; y ocho de ancho
pinta_el_movil:
	push de			;5537
	ld e,(hl)			;5538   ; su posicion, las dos coordenadas
	inc hl			;5539
	ld d,(hl)			;553a
	ld hl,06fe6h		;553b   ; la tabla de cuatro ajustes
	ld a,e			;553e
	and 006h		;553f   ; dos bits de la coordenada
	rra			;5541   ; por uno: son bytes
	call suma_a_a_hl		;5542   ; indexa la tabla
	ld a,(hl)			;5545
	pop hl			;5546
	push af			;5547   ; el ajuste se guarda: posicion_a_celda gasta A
	call posicion_a_celda		;5548   ; de posicion a celda de pantalla
	pop af			;554b   ; y vuelve para el volcado
	call vuelca_recortando_por_el_borde		;554c
	pop de			;554f
	ret			;5550

; ----------------------------------------------------------------------
; VOLCAR UN BLOQUE RECORTANDO POR EL BORDE DERECHO. Calcula cuantas columnas caben de verdad (`ld a,e / or 0e0h / neg` da lo que falta hasta el final de la fila) y, si no caben todas, recorta el ancho y se salta el resto. Es lo que deja que un dibujo asome a medias por el borde sin ensuciar la fila de abajo.
; ----------------------------------------------------------------------
vuelca_recortando_por_el_borde:
	set 6,d		;5551   ; la marca de escritura del VDP
	push de			;5553   ; el destino y el tamano pasan al juego alternativo
	push bc			;5554
	exx			;5555
	pop bc			;5556
	pop de			;5557
	ld b,c			;5558   ; el ancho, al contador del bucle
	ld hl,00020h		;5559   ; y una fila son 32 celdas
	exx			;555c
	ld d,a			;555d   ; el ajuste, a D
	ld a,e			;555e   ; la columna
	ld e,000h		;555f   ; y el byte bajo a cero: lo que se mira es la columna
	or 0e0h		;5561   ; lo que queda hasta el borde
	neg		;5563
	cp b			;5565   ; cabe entero?
	jr nc,L_556D		;5566
	push bc			;5568
	ld b,a			;5569   ; si no, se pinta solo lo que cabe
	pop af			;556a
	sub b			;556b   ; y el resto se salta
	ld e,a			;556c
L_556D:
	exx			;556d
L_556E:
	...
	push bc			;5573
L_5574:
	ld a,(hl)			;5574   ; el indice del dibujo
	inc hl			;5575
	add a,d			;5576   ; corrido por el desplazamiento de la pieza
	jr nc,saca_una_fila_al_vdp		;5577   ; si no desborda, va tal cual
	add a,d			;5579
	and 007h		;557a   ; y si desborda, se le da la vuelta a tres bits
saca_una_fila_al_vdp:
	out (c),a		;557c   ; al puerto de datos
	djnz L_5574		;557e
	ld a,e			;5580   ; el ancho, para saltar a la fila siguiente del dibujo
	call suma_a_a_hl		;5581
	pop bc			;5584
	exx			;5585
	ex de,hl			;5586
	add hl,de			;5587   ; y treinta y dos celdas en la VRAM
	ex de,hl			;5588
	djnz L_556E		;5589
	ei			;558b
	ret			;558c
...
; ----------------------------------------------------------------------
; LA CUENTA RAPIDA DE MOVILES: suma los tres bytes de estado y mira el nibble bajo. Si no da 0x0F, el hueco de sprite se libera. Es el mismo apano que con los recogibles, un contador que no puede descuadrarse porque se recalcula.
; ----------------------------------------------------------------------
suma_los_tres_moviles:
	ld hl,0e160h		;558d
	ld a,(hl)			;5590
	inc hl			;5591
	inc hl			;5592
	add a,(hl)			;5593   ; sumando de dos en dos bytes
	inc hl			;5594
	inc hl			;5595
	add a,(hl)			;5596
	and 00fh		;5597   ; el nibble bajo
	cp 00fh		;5599   ; el valor que dice que estan los tres
	jr z,pinta_el_movil_grande		;559b
	ld (ix+000h),000h		;559d   ; y si no, se libera el sprite
pinta_el_movil_grande:
	call libera_dos_huecos		;55a1
	push de			;55a4
	ld de,04800h		;55a5   ; su destino en la VRAM
	ld bc,0060ah		;55a8   ; seis por diez
	call pinta_bloque_en_su_sitio		;55ab
	pop de			;55ae
	ret			;55af

; ----------------------------------------------------------------------
; METER UN MOVIL EN EL BUFER DE SPRITES. Solo se dibuja dentro de una ventana: por encima de 0x3E no se pinta, y por debajo de 4 o a partir de 0x3C se borra. Los patrones van de dieciseis en dieciseis -0x47, 0x57, 0x67...- porque un sprite de 16x16 ocupa cuatro patrones.
; ----------------------------------------------------------------------
mete_el_movil_en_los_sprites:
	ld a,(hl)			;55b0
	cp 03eh		;55b1   ; fuera de la ventana por arriba
	ret nc			;55b3
	cp 03ch		;55b4   ; o pegado al borde
	ld hl,0e0b0h		;55b6   ; el bufer de sprites
	jr nc,borra_cinco_sprites		;55b9
	cp 004h		;55bb   ; o demasiado abajo
	jr c,borra_cinco_sprites		;55bd
	add a,016h		;55bf   ; el desplazamiento a su fila
	ld d,a			;55c1
	ld a,047h		;55c2   ; el primer patron
	ld b,002h		;55c4   ; dos sprites
	call apila_sprites_del_movil		;55c6
	ld b,003h		;55c9
	bit 7,(ix+000h)		;55cb   ; el bit 7 del movil: mira al otro lado
	jr z,apila_sprites_del_movil		;55cf
	ld (hl),0c3h		;55d1   ; y entonces el tercero es el patron 0xC3
	jr L_55D8		;55d3
apila_sprites_del_movil:
	ld (hl),a			;55d5   ; el patron
	add a,010h		;55d6   ; dieciseis mas: el siguiente sprite de 16x16
L_55D8:
	inc hl			;55d8
	ld (hl),d			;55d9   ; la fila
	inc hl			;55da   ; y cuatro bytes por sprite
	inc hl			;55db
	inc hl			;55dc
	djnz apila_sprites_del_movil		;55dd
	ret			;55df
borra_cinco_sprites:
	ex de,hl			;55e0
	ld b,005h		;55e1   ; cinco huecos
L_55E3:
	call libera_un_hueco_de_sprite		;55e3
	djnz L_55E3		;55e6
	ret			;55e8
quita_el_movil:
	ld a,(hl)			;55e9
	push af			;55ea
	call patrones_del_movil_quieto		;55eb   ; donde esta
	bit 7,(ix+000h)		;55ee   ; el bit 7
	pop bc			;55f2
	jr z,libera_un_hueco_de_sprite		;55f3
	ld a,(de)			;55f5
	cpl			;55f6   ; invertido y cruzado con el hueco
	or b			;55f7
	jp p,libera_un_hueco_de_sprite		;55f8
	ld a,b			;55fb
	jr L_561D		;55fc
libera_dos_huecos:
	call libera_un_hueco_de_sprite		;55fe

; ----------------------------------------------------------------------
; LIBERAR UN HUECO DE SPRITE poniendole 0xC3 en la fila, que es un valor fuera de pantalla, y saltando cuatro bytes al siguiente.
; ----------------------------------------------------------------------
libera_un_hueco_de_sprite:
	ld a,0c3h		;5601   ; la fila que lo saca de la pantalla
	ld (de),a			;5603
	inc de			;5604   ; cuatro bytes por sprite
	inc de			;5605
	inc de			;5606
	inc de			;5607
	ret			;5608
...
; ----------------------------------------------------------------------
; LOS PATRONES DE UN MOVIL, que cambian con la fase. En la de tipo 3 el dibujo alterna con el bit 2 de su estado -0x94 o 0x98-; las otras dos puertas traen sus parejas fijas.
; ----------------------------------------------------------------------
elige_los_patrones_del_movil:
	ld a,(0e052h)		;5609
	cp 003h		;560c   ; solo la fase 3 entra por aqui
	ret nz			;560e
	ld a,(hl)			;560f
	and 004h		;5610   ; el bit 2: dos dibujos alternos
	add a,094h		;5612   ; 0x94 o 0x98
	ld b,a			;5614
	ld a,(hl)			;5615
	ld c,006h		;5616
	ld hl,0a303h		;5618   ; su pareja de tablas
	jr L_562B		;561b
L_561D:
	ld bc,0780eh		;561d
	ld hl,0540eh		;5620
	jr L_562B		;5623
patrones_del_movil_quieto:
	ld bc,0a816h		;5625
	ld hl,0400eh		;5628
L_562B:
	ex de,hl			;562b
	add a,c			;562c   ; la columna, mas el ancho
	inc c			;562d
	inc c			;562e
	inc c			;562f
	cp c			;5630   ; si se sale por el borde
	jr nc,L_5635		;5631
	ld d,0c3h		;5633   ; la fila 0xC3 lo saca de la pantalla
L_5635:
	call mete_un_sprite		;5635
	ex de,hl			;5638
	ret			;5639

; ----------------------------------------------------------------------
; METER UN SPRITE en el bufer: fila, columna, patron y color, cuatro bytes de corrido.
; ----------------------------------------------------------------------
mete_un_sprite:
	ld (hl),d			;563a   ; la fila
	inc hl			;563b
	ld (hl),a			;563c   ; la columna
	inc hl			;563d
	ld (hl),b			;563e   ; el patron
	inc hl			;563f
	ld (hl),e			;5640   ; y el color
	inc hl			;5641
	ret			;5642
...
pinta_el_borde_del_decorado:
	...
	ld a,h			;5646
	or a			;5647   ; fuera de la primera pantalla
	jr nz,L_5677		;5648
	ld a,l			;564a
	add a,002h		;564b   ; margen de dos
	cp 004h		;564d
	jr c,L_566D		;564f
	ld a,l			;5651
	ld (0e14ch),a		;5652   ; la columna que entra
pinta_la_columna_que_entra:
	call que_dibujo_toca		;5655   ; que dibujo toca
	ld hl,0740fh		;5658   ; tres filas por ocho
	ld bc,00308h		;565b
	jr z,L_5666		;565e
	ld hl,0735ah		;5660   ; o tres por siete
	ld bc,00307h		;5663
L_5666:
	ld de,(0e14ch)		;5666
	jp elige_rectangulo_y_pinta		;566a
L_566D:
	ld bc,00803h		;566d   ; ocho por tres, por el otro camino
	ld de,(0e14ch)		;5670
	call pinta_bloque_en_su_sitio		;5674
L_5677:
	ld a,0ffh		;5677   ; y se marca como "nada que pintar"
	ld (0e14ch),a		;5679
	ret			;567c

; ----------------------------------------------------------------------
; EL DESPLAZAMIENTO DEL DECORADO. Suma +1 o -1 -el signo sale de `jp p` sobre el sentido- y de la posicion resultante salen a la vez la fila del sprite del borde (`add a,a / add a,a / add a,014h`) y su patron; el valor 0x34 se cambia por 0xF8, que es el unico caso especial.
; ----------------------------------------------------------------------
desplaza_el_decorado:
	call que_direccion_se_pulsa		;567d   ; el sentido que pulsa el jugador
	ld c,a			;5680
	ld b,000h		;5681   ; la parte alta del incremento
	jp p,corre_el_decorado_bc		;5683
	dec b			;5686   ; negativo: -1
corre_el_decorado_bc:
	ld hl,(0e055h)		;5687   ; la posicion del decorado
	add hl,bc			;568a   ; se le suma
	ld (0e055h),hl		;568b
	ld a,h			;568e
	add a,a			;568f   ; el byte alto por cuatro
	add a,a			;5690
	add a,014h		;5691   ; mas veinte: la fila del sprite
	cp 034h		;5693   ; el caso especial
	ld b,a			;5695
	jr nz,L_569A		;5696
	ld b,0f8h		;5698   ; se sustituye
L_569A:
	ld a,l			;569a
	ld de,0b60bh		;569b
	ld hl,0e0f0h		;569e
	call mete_un_sprite		;56a1
	add a,010h		;56a4   ; dieciseis mas: el segundo sprite
	jr nc,L_56AA		;56a6
	ld d,0c3h		;56a8   ; y si se sale, fuera de pantalla
L_56AA:
	bit 7,b		;56aa
	jr nz,L_56B0		;56ac
	ld b,010h		;56ae
L_56B0:
	jp mete_un_sprite		;56b0
cuenta_el_paso:
	call direccion_pulsada_cruda		;56b3   ; la direccion pulsada, sin filtrar
	ret z			;56b6   ; nada pulsado
	bit 3,a		;56b7   ; el bit 3: hacia un lado o hacia el otro
	ld hl,0e14ah		;56b9
	ld a,(0e1d4h)		;56bc   ; las dos posiciones que van juntas
	ld b,a			;56bf
	ld a,(0e1cah)		;56c0
	jr z,L_56CA		;56c3
	dec (hl)			;56c5   ; hacia un lado
	dec a			;56c6
	dec b			;56c7
	jr L_56CD		;56c8
L_56CA:
	inc (hl)			;56ca   ; o hacia el otro, las tres a la vez
	inc a			;56cb
	inc b			;56cc
L_56CD:
	ld (0e1cah),a		;56cd
...
; ----------------------------------------------------------------------
; LA FIGURA GRANDE DEL DECORADO. Elige uno de cuatro dibujos con los bits 1 y 2 de su posicion -`and 006h` sobre una tabla de punteros-, lo pinta, y despues recoloca OCHO sprites a su alrededor con desplazamientos fijos: -0x2E, +0x1E y +9 desde la misma referencia. Es como se le pegan los adornos sin guardarlos en el dibujo.
; ----------------------------------------------------------------------
pinta_la_figura_grande:
	ld a,(hl)			;56d4
	and 006h		;56d5   ; dos bits: cuatro variantes
	ld hl,06acbh		;56d7   ; la tabla de dibujos
	call suma_a_a_hl		;56da
	ld e,(hl)			;56dd
	inc hl			;56de
	ld d,(hl)			;56df
	ex de,hl			;56e0
	ld de,(0e14ah)		;56e1   ; la posicion
	call posicion_a_celda		;56e5   ; convertida en celda
	call vuelca_la_figura		;56e8
	ld a,(0e14ah)		;56eb
	sub 02eh		;56ee   ; 46 a la izquierda
	res 0,a		;56f0   ; y redondeado a par
	ld (0e0e1h),a		;56f2
	ld (0e0e9h),a		;56f5
	add a,01eh		;56f8   ; treinta a la derecha
	ld (0e0e5h),a		;56fa
	ld (0e0edh),a		;56fd
	add a,009h		;5700   ; nueve mas
	ld (0e121h),a		;5702
	ld (0e129h),a		;5705
	sub 02eh		;5708   ; y de vuelta al principio
	ld (0e125h),a		;570a
	ld (0e12dh),a		;570d
	ret			;5710
...
vuelca_la_figura:
	di			;5711
	set 6,d		;5712   ; la marca de escritura del VDP
	ld a,015h		;5714
	ld b,020h		;5716   ; treinta y dos celdas
	call vuelca_con_referencias		;5718
	inc hl			;571b
	inc hl			;571c
	ld c,006h		;571d   ; seis grupos
	ld a,020h		;571f
L_5721:
	push hl			;5721
	ex af,af'			;5722
	...
	ex af,af'			;5728
	ld b,005h		;5729   ; de cinco
	call vuelca_con_referencias		;572b
	pop hl			;572e
	ld b,006h		;572f
L_5731:
	ld a,(hl)			;5731   ; un indice de patron
	inc hl			;5732   ; el siguiente
	exx			;5733   ; al juego alternativo, donde esta el puerto
	out (c),a		;5734   ; y al VDP
	exx			;5736   ; y de vuelta
	...
	inc hl			;5745   ; dos bytes mas alla
	inc hl			;5746
	ld a,02fh		;5747   ; el patron de relleno del hueco
	dec c			;5749   ; una franja menos
	jr nz,L_5721		;574a   ; y a por la siguiente
	ld a,0eah		;574c   ; el desplazamiento al bloque de abajo
	dec h			;574e   ; la pagina de arriba
	call suma_a_a_hl		;574f   ; sumado
	ld a,0c0h		;5752
	dec d			;5754   ; una fila menos
	ld b,00fh		;5755   ; quince celdas
	call vuelca_con_referencias		;5757   ; volcadas
	ld a,008h		;575a   ; ocho mas alla
	call suma_a_a_hl		;575c   ; sumado
	ld a,031h		;575f
	ld b,00fh		;5761   ; y otras quince en la fila siguiente
	call vuelca_con_referencias		;5763   ; volcadas
	dec hl			;5766   ; el byte de antes
	ld a,(hl)			;5767   ; el ultimo indice leido
	sub 0b8h		;5768   ; menos 0xB8
	and 00ch		;576a   ; dos bits: cuatro variantes
	add a,a			;576c   ; por dos
	add a,080h		;576d   ; y con 0xE2 delante sale una direccion del bufer de RAM
	ld l,a			;576f   ; en L
	ld h,0e2h		;5770   ; y la pagina del bufer
	ld de,02288h		;5772   ; el destino en la VRAM
	ld c,008h		;5775   ; ocho celdas
	call que_dibujo_toca		;5777   ; el tipo de fase decide si hay una segunda copia
	jr nz,L_5788		;577a   ; en los demas tipos no la hay
	push hl			;577c   ; se guarda el origen
	ld d,02ah		;577d   ; la fila de la otra copia
	call vuelca_bloque_a_vram		;577f   ; volcada
	pop hl			;5782   ; se recupera
	ld de,03288h		;5783   ; y el otro destino
	ld c,008h		;5786   ; ocho celdas
L_5788:
...
vuelca_franja_con_referencias:
	ld a,(hl)			;578e   ; el indice
...
L_5799:
	...
	ret			;57a7
que_dibujo_toca:
	ld a,(0e052h)		;57a8   ; el tipo de fase
	or a			;57ab
	ret z			;57ac
...
; ----------------------------------------------------------------------
; AVANZAR EL PUNTERO DE VRAM SIN SALIRSE DE LA FILA. Parte el desplazamiento en su trozo de fila (`and 01fh`) y su trozo de columna, los suma por separado y los vuelve a juntar: asi un avance grande no se lleva por delante el principio de la fila de abajo.
; ----------------------------------------------------------------------
avanza_sin_salir_de_la_fila:
	push hl			;57b3
	ld l,a			;57b4
	and 01fh		;57b5   ; la parte de columna
	ld h,a			;57b7
	xor l			;57b8   ; y la de fila
	add a,e			;57b9   ; sumada
	jr nc,L_57BD		;57ba
	inc d			;57bc   ; con acarreo a la pagina
L_57BD:
	and 0e0h		;57bd
	ld l,a			;57bf
	ld a,e			;57c0   ; las columnas, aparte
	add a,h			;57c1
	and 01fh		;57c2
	or l			;57c4   ; y se vuelven a juntar
	ld e,a			;57c5
	pop hl			;57c6
	call prepara_escritura_vram		;57c7   ; con el puntero ya recolocado
	ret			;57ca

; ----------------------------------------------------------------------
; EL BUCLE DE LOS SEIS RECOGIBLES: cada uno tiene su estado en 0xE180, su posicion en 0xE170 y su hueco de sprite en 0xE0FC. Si el estado no es cero se mueve, y si lo es se intenta arrancar.
; ----------------------------------------------------------------------
mueve_los_seis_recogibles:
	ld ix,0e180h		;57cb   ; el estado de cada uno
	ld de,0e0fch		;57cf   ; su hueco de sprite
	ld hl,0e170h		;57d2   ; y su posicion
	ld b,006h		;57d5   ; seis
L_57D7:
	push bc			;57d7
	push hl			;57d8
	...
	or a			;57dc   ; parado?
	push af			;57dd
	call nz,mueve_un_recogible		;57de   ; en marcha: se mueve
	pop af			;57e1
	call z,arranca_movil_activo		;57e2   ; parado: se mira si le toca salir
	pop hl			;57e5
	pop bc			;57e6
	inc hl			;57e7   ; dos bytes de posicion
	inc hl			;57e8
	inc ix		;57e9   ; y dos de estado
	inc ix		;57eb
	djnz L_57D7		;57ed

; ----------------------------------------------------------------------
; EL SEPTIMO, que solo existe en las fases de tipo 3. Su dibujo es de 3x3 y su patron cambia con el contador de cuadros -uno de cada dos, `rra / and 007h / add a,094h`- pero SOLO si el jugador esta pulsando algo: parado, se queda quieto.
; ----------------------------------------------------------------------
el_septimo_de_la_fase_3:
	ld a,(0e052h)		;57ef   ; el tipo de fase
	cp 003h		;57f2   ; solo la 3 lo tiene
	ret nz			;57f4
	ld a,(ix+000h)		;57f5
	or a			;57f8
	ret z			;57f9
	ld de,0e0f8h		;57fa
	cp 07eh		;57fd   ; el valor 0x7E lo trata aparte
	jr z,pinta_el_septimo		;57ff
	ld a,(0e139h)		;5801   ; el bit 2 y 3 de (0xE139)
	and 00ch		;5804
	jr nz,L_5837		;5806
pinta_el_septimo:
	ld a,038h		;5808
	ld de,0a038h		;580a   ; la fila y el patron
	cp (hl)			;580d
	push de			;580e
	call nz,coloca_el_septimo		;580f
	pop de			;5812
	ld bc,00303h		;5813   ; tres por tres
	ld hl,0724ah		;5816   ; su dibujo
	call pinta_rectangulo		;5819
	ld hl,0e0f8h		;581c
	ld (hl),0a3h		;581f   ; el sprite que lleva encima
	inc hl			;5821
	ld (hl),03ch		;5822
	inc hl			;5824
	call direccion_pulsada_cruda		;5825   ; se esta pulsando algo?
	jr z,L_5830		;5828
	ld a,(0e003h)		;582a   ; el contador de cuadros
	rra			;582d   ; uno de cada dos
	and 007h		;582e
L_5830:
	add a,094h		;5830   ; y de ahi el patron, de 0x94 en adelante
	ld (hl),a			;5832
	inc hl			;5833
	ld (hl),003h		;5834
	ret			;5836
L_5837:
	ld a,(hl)			;5837
	sub 003h		;5838
	jr nc,pinta_un_recogible		;583a
	jr L_589F		;583c
coloca_el_septimo:
	ld (0e17ch),de		;583e   ; su posicion, apuntada
	ld e,000h		;5842
	ld bc,00e03h		;5844   ; catorce por tres
	call pinta_bloque_en_su_sitio		;5847
	call suma_los_estados		;584a
	ret nz			;584d
	call posicion_en_el_decorado		;584e   ; donde cae en el decorado
	ret nc			;5851
	jp baja_los_tres_recogibles		;5852

; ----------------------------------------------------------------------
; MOVER UNO DE LOS SEIS, con el mismo paso y medio que los moviles: el sentido del jugador mas el bit 0 del contador de cuadros convertido en 0 o -1. La fase de tipo 2 tiene su propia rutina.
; ----------------------------------------------------------------------
mueve_un_recogible:
	ld a,(0e052h)		;5855   ; el tipo de fase
	cp 002h		;5858
	jr z,mueve_el_recogible_de_la_fase_2		;585a   ; la fase 2 va por otro lado
	call que_direccion_se_pulsa		;585c   ; el sentido que pulsa el jugador
	ld b,a			;585f
	ld a,(0e003h)		;5860   ; el contador de cuadros
	ld c,a			;5863
	rra			;5864   ; su bit 0
	sbc a,a			;5865   ; como 0 o -1
	add a,b			;5866
	add a,(hl)			;5867   ; y sumado a la posicion
	cp 002h		;5868
	jr c,apaga_el_recogible		;586a
	inc hl			;586c
	ld (hl),0a0h		;586d   ; se marca en la fila de al lado
	dec hl			;586f

; ----------------------------------------------------------------------
; PINTAR UN RECOGIBLE. Por encima de 0x85 -o sea por debajo en la pantalla- se libera su sprite y se pinta la version quieta (0x72D2); por debajo lleva sprite y el dibujo elegido de la tabla de 0x7242.
; ----------------------------------------------------------------------
pinta_un_recogible:
	ld (hl),a			;5870
	ld a,(0e130h)		;5871   ; la posicion del jugador
	cp 085h		;5874   ; el umbral
	jr nc,L_588C		;5876
	push hl			;5878
	call elige_los_patrones_del_movil		;5879   ; sus patrones
	pop hl			;587c
	push de			;587d
	ld e,(hl)			;587e
	inc hl			;587f
	ld d,(hl)			;5880
	ld hl,07242h		;5881   ; la tabla de cuatro dibujos
	ld bc,00305h		;5884   ; tres por cinco
	call elige_rectangulo_y_pinta		;5887
	pop de			;588a
	ret			;588b
L_588C:
	call libera_un_hueco_de_sprite		;588c   ; ya no lleva sprite
	push de			;588f
	ld e,(hl)			;5890
	inc hl			;5891
	ld d,(hl)			;5892
	ld hl,072d2h		;5893   ; y se pinta quieto
	ld bc,00305h		;5896
L_5899:
	call pinta_rectangulo		;5899
	pop de			;589c
	ret			;589d
apaga_el_recogible:
	ld (hl),a			;589e
L_589F:
	push de			;589f
	ld bc,00503h		;58a0   ; cinco por tres
L_58A3:
	ld e,(hl)			;58a3
	inc hl			;58a4
	ld d,(hl)			;58a5
	call pinta_bloque_en_su_sitio		;58a6
	pop de			;58a9
	call suma_los_estados		;58aa   ; mira si quedan mas
	cp 00fh		;58ad   ; si el reparto no cuadra, se deja
	jr z,L_58B5		;58af
	ld (ix+000h),000h		;58b1   ; y si no, se apaga del todo
L_58B5:
	jp libera_un_hueco_de_sprite		;58b5

; ----------------------------------------------------------------------
; SUMAR LOS CINCO ESTADOS de los recogibles y quedarse con el nibble bajo: es la cuenta rapida de cuantos quedan sin recoger, sin un contador aparte que pueda descuadrarse.
; ----------------------------------------------------------------------
suma_los_estados:
	ld hl,0e180h		;58b8
	ld b,005h		;58bb   ; cinco
	ld a,(hl)			;58bd
L_58BE:
	inc hl			;58be
	inc hl			;58bf
	add a,(hl)			;58c0   ; sumando de dos en dos bytes
	djnz L_58BE		;58c1
	and 00fh		;58c3   ; y el nibble bajo es la cuenta
	ret			;58c5

; ----------------------------------------------------------------------
; EL RECOGIBLE DE LA FASE 2, que se mueve solo. Si el suelto de 0xE1B0 esta a 0xFE no hay referencia; si no, se mide contra el con una ventana de 0x40 -veinte pixeles a cada lado-. Y cuando no hay nada pulsado, baja uno cada cuatro cuadros.
; ----------------------------------------------------------------------
mueve_el_recogible_de_la_fase_2:
	push de			;58c6
	push ix		;58c7
	pop bc			;58c9
	ld a,(0e1b0h)		;58ca   ; el suelto
	cp 0feh		;58cd   ; 0xFE: no hay referencia
	jr z,L_58DA		;58cf
	ld d,a			;58d1
	ld a,(hl)			;58d2
	sub d			;58d3
	add a,020h		;58d4   ; centrado
	cp 040h		;58d6   ; sesenta y cuatro de ventana
	jr c,L_58E3		;58d8
L_58DA:
	call direccion_pulsada_cruda		;58da   ; se esta pulsando algo?
	jr z,baja_uno_de_cada_dos		;58dd
	bit 3,a		;58df
	jr nz,L_58EA		;58e1
L_58E3:
	ld a,(0e003h)		;58e3   ; el contador de cuadros
	and 003h		;58e6   ; uno de cada cuatro
	jr nz,L_58EB		;58e8
L_58EA:
	dec (hl)			;58ea   ; y entonces baja uno
L_58EB:
	ld e,(hl)			;58eb
	inc hl			;58ec
	ld d,050h		;58ed   ; la fila fija
	ld (hl),d			;58ef
	dec hl			;58f0
	ld a,(hl)			;58f1
	cp 002h		;58f2
	jr c,lo_deja_quieto		;58f4
	and 00eh		;58f6   ; tres bits de la posicion
	add a,a			;58f8   ; por cuatro: cada pieza son cuatro bytes
	add a,a			;58f9
	ld hl,05a2fh		;58fa   ; las dieciseis piezas de tres tiles
	call suma_a_a_hl		;58fd
	ld bc,00204h		;5900   ; dos por cuatro
	jp L_5899		;5903
lo_deja_quieto:
	ld (hl),001h		;5906
	ld bc,00302h		;5908   ; tres por dos
	jr L_58A3		;590b
baja_uno_de_cada_dos:
	ld a,(0e003h)		;590d   ; el contador de cuadros
	and 001h		;5910   ; uno de cada dos
	jr nz,L_58EB		;5912
	jr L_58EA		;5914
L_5916:
	dec (hl)			;5916
...
; ----------------------------------------------------------------------
; EL SUELTO DE LA FASE 2. Mientras (0xE1BA) no sea cero se limita a descontar; cuando llega, decide entre arrancar de nuevo -poniendo la rampa de 0x59FA y la marca 0xFE- o seguir moviendose. El 0xFE es lo que las cajas de contacto leen como "aqui no hay nada".
; ----------------------------------------------------------------------
mueve_el_suelto:
	ld hl,0e1bah		;5918   ; su plazo
	ld a,(hl)			;591b
	or a			;591c   ; todavia corre
	jr nz,L_5916		;591d
	ld de,0e1b0h		;591f   ; el suelto
	ld a,(de)			;5922
	cp 002h		;5923   ; por debajo de dos, se reinicia
	jr c,L_592B		;5925
	cp 0ffh		;5927   ; y 0xFF tambien
	jr c,L_5947		;5929
L_592B:
	ld (hl),080h		;592b   ; el plazo largo
	xor a			;592d
	ld (0e1b4h),a		;592e
	ld (0e1b5h),a		;5931
	ld hl,059fah		;5934   ; la rampa de catorce valores
	ld (0e1b6h),hl		;5937   ; apuntada
	ld a,0feh		;593a   ; y la marca de "no hay nada"
	ld (de),a			;593c
	inc de			;593d
	ld a,050h		;593e   ; la fila fija
	ld (de),a			;5940
	ld (0e217h),a		;5941
	jp mete_el_suelto_en_los_sprites		;5944
L_5947:
	ex de,hl			;5947   ; aqui se mueve de verdad
	ld a,(0e1b4h)		;5948   ; esta bloqueado?
	or a			;594b
	jr nz,recorre_la_rampa_del_suelto		;594c
	call direccion_pulsada_cruda		;594e   ; se esta pulsando algo?
	jr z,L_5958		;5951
	bit 3,a		;5953   ; el bit 3: hacia un lado
	jr z,baja_uno_de_cada_dos_cuadros		;5955
	dec (hl)			;5957   ; dos de golpe
L_5958:
	dec (hl)			;5958
L_5959:
	ld a,(hl)			;5959
	inc hl			;595a
	inc hl			;595b
	bit 2,a		;595c   ; los bits 1 y 2 eligen el dibujo
	jr z,L_5968		;595e
	bit 1,a		;5960
	jr nz,L_596C		;5962
	ld (hl),004h		;5964   ; uno
	jr L_596E		;5966
L_5968:
	ld (hl),000h		;5968   ; otro
	jr L_596E		;596a
L_596C:
	ld (hl),008h		;596c   ; y el tercero
L_596E:
	call mete_el_suelto_en_los_sprites		;596e

; ----------------------------------------------------------------------
; EL SUELTO CONTRA LOS SEIS RECOGIBLES, con veinticuatro pixeles de margen. Es lo que decide si choca con alguno de ellos.
; ----------------------------------------------------------------------
mira_los_seis_contra_el_suelto:
	ld hl,0e1b0h		;5971
	ld de,0e170h		;5974
	ld b,006h		;5977   ; seis
L_5979:
	...
	ld a,(de)			;597a
	dec de			;597b
	cp 002h		;597c   ; los que estan por debajo de dos no cuentan
	jr c,L_598B		;597e
	ld a,(de)			;5980
	or a			;5981
	jr z,L_598B		;5982
	add a,018h		;5984   ; veinticuatro de desplazamiento
	sub (hl)			;5986
	cp 002h		;5987   ; dos pixeles de margen
	jr c,bloquea_el_suelto		;5989
L_598B:
	inc de			;598b   ; dos bytes por recogible
	inc de			;598c
	djnz L_5979		;598d
	ret			;598f
bloquea_el_suelto:
	inc hl			;5990
	inc hl			;5991
	ld (hl),00ch		;5992   ; el plazo de bloqueo
	ld a,001h		;5994
	ld (0e1b4h),a		;5996   ; y la marca de bloqueado
	ret			;5999
baja_uno_de_cada_dos_cuadros:
	ld a,(0e003h)		;599a   ; el contador de cuadros
	and 001h		;599d   ; uno de cada dos
	jr nz,L_5959		;599f
	dec (hl)			;59a1
	jr L_5959		;59a2

; ----------------------------------------------------------------------
; EL SUELTO BLOQUEADO RECORRE SU RAMPA. (0xE1B5) dice en que sentido -y con el bit 0 se decide si el valor de la rampa se suma o se RESTA, con `neg`-, y la rampa se recorre hacia delante o hacia atras. Los valores 0xFE y 0xFF son los topes; al llegar al 0xFF se da la vuelta y al 0xFE se desbloquea.
; ----------------------------------------------------------------------
recorre_la_rampa_del_suelto:
	ld a,(0e1b5h)		;59a4   ; el sentido
	ld b,a			;59a7
	inc hl			;59a8
	ld de,(0e1b6h)		;59a9   ; por donde va la rampa
	bit 0,b		;59ad   ; el bit 0 decide el signo
	ld a,(de)			;59af
	jr nz,L_59B4		;59b0
	neg		;59b2   ; restar en vez de sumar
L_59B4:
	add a,(hl)			;59b4
	ld (hl),a			;59b5   ; y se aplica
	dec hl			;59b6
	dec (hl)			;59b7   ; dos hacia abajo
	dec (hl)			;59b8
	ex de,hl			;59b9
	bit 0,b		;59ba
	jr nz,L_59C1		;59bc
	inc hl			;59be   ; la rampa avanza
	jr L_59C2		;59bf
L_59C1:
	dec hl			;59c1   ; o retrocede
L_59C2:
	ld a,(hl)			;59c2
	inc a			;59c3   ; 0xFF: el tope
	jr nz,desbloquea_el_suelto		;59c4
	dec hl			;59c6
	inc a			;59c7
	ld (0e1b5h),a		;59c8   ; se da la vuelta al sentido
L_59CB:
	ld (0e1b6h),hl		;59cb

; ----------------------------------------------------------------------
; METER EL SUELTO EN EL BUFER DE SPRITES. Si su posicion es 0xFE -la marca de "no hay nada"- se le pone la fila 0xC3, que lo saca de la pantalla: el mismo valor que usa el juego para liberar cualquier hueco.
; ----------------------------------------------------------------------
mete_el_suelto_en_los_sprites:
	ld hl,0e1b0h		;59ce
	ld b,(hl)			;59d1
	inc hl			;59d2
	ld c,(hl)			;59d3
	ld a,0feh		;59d4   ; la marca de "no hay nada"
	cp b			;59d6
	jr nz,L_59DB		;59d7
	ld c,0c3h		;59d9   ; y entonces, fuera de la pantalla
L_59DB:
	inc hl			;59db
	ld a,(hl)			;59dc
	inc hl			;59dd
	ld e,(hl)			;59de
	ld hl,0e0f8h		;59df
	add a,084h		;59e2   ; el patron, de 0x84 en adelante
	ld (hl),c			;59e4   ; fila, columna, patron y color
	inc hl			;59e5
	ld (hl),b			;59e6
	inc hl			;59e7
	ld (hl),a			;59e8
	...
	ld (hl),e			;59ea
	ret			;59eb
desbloquea_el_suelto:
	inc a			;59ec   ; 0xFE: el otro tope de la rampa
	jr nz,L_59CB		;59ed
	xor a			;59ef
	ld (0e1b4h),a		;59f0   ; se desbloquea
	ld (0e1b5h),a		;59f3
	inc hl			;59f6
	jr L_59CB		;59f7
...
mira_los_seis_contra_el_jugador:
	ld b,006h		;5a08   ; seis
	ld de,0e170h		;5a0a   ; sus posiciones
	ld hl,(0e130h)		;5a0d   ; y la del jugador, las dos coordenadas de un tiron
L_5A10:
	ld a,(0e134h)		;5a10   ; el estado del jugador
	and a			;5a13
	call z,caja_contra_el_jugador		;5a14   ; solo se comprueba corriendo
	ret c			;5a17
	inc de			;5a18   ; el siguiente
	djnz L_5A10		;5a19
	ld de,0e1b0h		;5a1b   ; y al final, el suelto

; ----------------------------------------------------------------------
; LA CAJA CONTRA EL JUGADOR: 0x1C por 0x28, centrada con `add a,00eh` y `add a,010h` -o sea, catorce y dieciseis a cada lado-. Devuelve el acarreo puesto si toca, que es lo que corta el bucle de arriba.
; ----------------------------------------------------------------------
caja_contra_el_jugador:
	ld a,(de)			;5a1e
	inc de			;5a1f
	sub h			;5a20
	add a,00eh		;5a21   ; catorce, o sea centrada
	cp 01ch		;5a23   ; veintiocho de ancho
	ret nc			;5a25
	ld a,(de)			;5a26
	sub l			;5a27
	add a,010h		;5a28   ; dieciseis
	cp 028h		;5a2a   ; y cuarenta en la otra
	ret nc			;5a2c
	scf			;5a2d   ; acarreo: hay contacto
	ret			;5a2e
...
saca_la_siguiente_de_la_cola:
	...
	ld (hl),a			;5ab9
	ret nc			;5aba   ; todavia no toca
	ld hl,0e25ch		;5abb
	ld b,004h		;5abe   ; cuatro huecos
L_5AC0:
	ld a,(hl)			;5ac0
	or a			;5ac1
	jr z,L_5ACB		;5ac2   ; libre
	inc hl			;5ac4   ; cuatro bytes por hueco
	inc hl			;5ac5
	inc hl			;5ac6
	inc hl			;5ac7
	djnz L_5AC0		;5ac8
	ret			;5aca
L_5ACB:
	ld de,(0e26ch)		;5acb   ; por donde va el guion
	ld a,(de)			;5acf
	and 018h		;5ad0   ; dos bits del primer byte
	rra			;5ad2
	rra			;5ad3
	rra			;5ad4
	inc a			;5ad5   ; uno mas
	ld b,a			;5ad6
	ld a,03ah		;5ad7   ; y de ahi la fila, de 0x3A en adelante
L_5AD9:
	add a,01eh		;5ad9   ; sumando 0x1E por cada paso
	djnz L_5AD9		;5adb
	ld (hl),a			;5add
	ld a,(de)			;5ade
	inc hl			;5adf
	...
	inc de			;5ae8
	ld a,(de)			;5ae9
	ld (0e26eh),a		;5aea   ; lo que falta para la siguiente
	inc de			;5aed
	ld (0e26ch),de		;5aee   ; y el guion queda apuntado
	ld hl,(0e26ch)		;5af2
	ld a,(hl)			;5af5
	inc a			;5af6   ; 0xFF: fin del guion
	jr nz,L_5AFC		;5af7
	ld hl,(0e25ah)		;5af9   ; y vuelve al principio
L_5AFC:
	ld (0e26ch),hl		;5afc
	ret			;5aff
mueve_lo_que_se_empuja:
	call direccion_pulsada_cruda		;5b00   ; lo que se esta pulsando
	jr z,avanza_con_la_velocidad		;5b03
	bit 2,a		;5b05   ; el bit 2: hacia donde
	ld a,(0e270h)		;5b07
	jr z,acelera		;5b0a
	dec a			;5b0c   ; uno menos
	cp 020h		;5b0d   ; el tope
	jr nc,L_5B13		;5b0f
	ld a,020h		;5b11   ; el tope por abajo
L_5B13:
	ld (0e270h),a		;5b13

; ----------------------------------------------------------------------
; EL AVANCE CON VELOCIDAD ACUMULADA. (0xE270) es la velocidad y (0xE271) el resto: se suman, se guardan los seis bits de abajo del resto, y los dos que se desbordan (`add hl,hl` dos veces y quedarse con H) son los pixeles que se mueve todo este cuadro. Es un acumulador de punto fijo hecho con dos bytes y ninguna division.
; ----------------------------------------------------------------------
avanza_con_la_velocidad:
	ld hl,0e270h		;5b16   ; la velocidad
	ld a,(hl)			;5b19   ; leida
	inc hl			;5b1a   ; el byte siguiente
	add a,(hl)			;5b1b   ; mas el resto que quedaba
	ld e,a			;5b1c   ; en E
	ld d,000h		;5b1d   ; y DE queda con el total
	and 03fh		;5b1f   ; y el resto nuevo, seis bits
	ld (hl),a			;5b21   ; guardado
	ex de,hl			;5b22   ; se cambian
	add hl,hl			;5b23   ; dos veces por dos
	add hl,hl			;5b24
	ld c,h			;5b25   ; y lo que se desborda son los pixeles
	ld hl,0e14ah		;5b26   ; la referencia del decorado
	ld a,(hl)			;5b29   ; el decorado se corre
	sub c			;5b2a
	ld (hl),a			;5b2b   ; guardada
	...
	ld (hl),a			;5b3c   ; guardada
	push bc			;5b3d   ; se guarda otra vez
	call pinta_la_columna_que_entra		;5b3e   ; y se pinta la columna que entra
	pop bc			;5b41   ; se recupera
L_5B42:
	push bc			;5b42
	ld b,000h		;5b43   ; el byte alto, a cero
	ld a,c			;5b45   ; y el avance en el bajo
	or a			;5b46
	jr z,L_5B4E		;5b47
	neg		;5b49   ; al reves para el desplazamiento del decorado
	ld c,a			;5b4b
	ld b,0ffh		;5b4c   ; y con el byte alto a 0xFF: negativo
L_5B4E:
	call corre_el_decorado_bc		;5b4e
	pop bc			;5b51
corre_la_cola_con_la_velocidad:
	ld hl,0e25ch		;5b52   ; los cuatro huecos de la cola
	ld b,004h		;5b55
L_5B57:
	push bc			;5b57
	push hl			;5b58
	...
	inc hl			;5b5e
	ld a,(hl)			;5b5f
	sub c			;5b60   ; se corre lo que toque
	ld (hl),a			;5b61
	ld a,003h		;5b62   ; al llegar a tres
	cp (hl)			;5b64
	pop hl			;5b65
	push hl			;5b66
	call nc,apaga_un_hueco_de_la_cola		;5b67   ; se apaga
L_5B6A:
	pop hl			;5b6a
	pop bc			;5b6b
	inc hl			;5b6c   ; cuatro bytes por hueco
	inc hl			;5b6d
	inc hl			;5b6e
	inc hl			;5b6f
	djnz L_5B57		;5b70
...
acelera:
	inc a			;5b73   ; uno mas
	cp 080h		;5b74   ; el tope por arriba
	jr c,L_5B7A		;5b76
	ld a,080h		;5b78   ; y se clava ahi
L_5B7A:
	jr L_5B13		;5b7a
pinta_la_cola:
	ld hl,0e25ch		;5b7c   ; los cuatro huecos
	ld b,004h		;5b7f
L_5B81:
	push hl			;5b81   ; se guardan los dos
	push bc			;5b82
	ld a,(hl)			;5b83   ; el estado del hueco
	or a			;5b84
	jr z,pasa_al_hueco_siguiente		;5b85   ; vacio: al siguiente
	ex af,af'			;5b87   ; se guarda
	inc hl			;5b88   ; el byte siguiente
	ld d,(hl)			;5b89   ; la posicion del hueco
	inc hl			;5b8a   ; el siguiente
	ld e,(hl)			;5b8b
	ld c,e			;5b8c   ; la columna, tambien en C
	inc hl			;5b8d   ; y el ultimo
	ld b,(hl)			;5b8e
	call posicion_a_celda		;5b8f   ; convertida en celda
	push de			;5b92   ; se guardan
	push bc			;5b93
	ld a,c			;5b94   ; la columna
	and 006h		;5b95   ; tres bits de la columna
	ld c,a			;5b97   ; en C
	add a,a			;5b98   ; dos veces por dos
	add a,a			;5b99
	add a,c			;5b9a   ; mas ella misma
	rra			;5b9b   ; entre dos: queda el indice
	ld hl,07527h		;5b9c   ; la tabla de veinte indices
	call suma_a_a_hl		;5b9f   ; indexada
	push hl			;5ba2   ; se guarda
	call pinta_dos_o_tres_celdas		;5ba3   ; la fila de arriba
	ld a,c			;5ba6   ; la columna
	add a,00fh		;5ba7   ; quince mas para la de abajo
	ex af,af'			;5ba9   ; se guarda
	pop hl			;5baa   ; se recuperan los tres
	pop bc			;5bab
	pop de			;5bac
	ld a,020h		;5bad   ; y la fila siguiente, 32 celdas
	call suma_a_a_de		;5baf   ; sumadas al destino
	call pinta_dos_o_tres_celdas		;5bb2   ; y se pinta la de abajo
pasa_al_hueco_siguiente:
	pop bc			;5bb5   ; se recuperan
	pop hl			;5bb6
	inc hl			;5bb7   ; cuatro bytes por hueco
	inc hl			;5bb8
	inc hl			;5bb9
	inc hl			;5bba
	djnz L_5B81		;5bbb   ; y a por el siguiente
	ret			;5bbd

; ----------------------------------------------------------------------
; PINTAR LA FILA DE UNA PIEZA DE LA COLA, de dos o tres celdas segun el ancho que traiga B. Al indice de la tabla se le SUMA C, que es el desplazamiento de patrones de esa pieza: la misma tabla de veinte valores sirve para todas las piezas cambiando ese sumando.
; ----------------------------------------------------------------------
pinta_dos_o_tres_celdas:
	ex af,af'			;5bbe
	ld c,a			;5bbf   ; el desplazamiento de patrones
pinta_pieza_de_dos_filas:
	push bc			;5bc0
	ld b,002h		;5bc1   ; dos celdas
	call pinta_una_fila_de_la_pieza		;5bc3
	pop bc			;5bc6
	dec b			;5bc7
	jr z,pinta_una_celda		;5bc8
	dec b			;5bca
L_5BCB:
	ld a,(hl)			;5bcb
	add a,c			;5bcc   ; el indice, corrido por el desplazamiento
	call escribe_en_vram		;5bcd
	inc de			;5bd0
	djnz L_5BCB		;5bd1
	inc hl			;5bd3
	ld a,(hl)			;5bd4
	dec a			;5bd5   ; un uno en la tabla marca que no hay hueco
	jr z,L_5BD9		;5bd6
	dec de			;5bd8   ; y entonces se retrocede una celda
L_5BD9:
	ld b,002h		;5bd9
pinta_una_fila_de_la_pieza:
	ld a,(hl)			;5bdb   ; el indice de la tabla
	add a,c			;5bdc   ; corrido por el desplazamiento de la pieza
	call escribe_en_vram		;5bdd
	inc hl			;5be0
	inc de			;5be1
	djnz pinta_una_fila_de_la_pieza		;5be2
	ret			;5be4
pinta_una_celda:
	inc hl			;5be5
	inc hl			;5be6
	ld a,(hl)			;5be7
	or a			;5be8   ; cero: no hay nada que pintar
	jr z,L_5BEC		;5be9
	dec de			;5beb
L_5BEC:
	ld a,(hl)			;5bec
	add a,c			;5bed   ; el indice corrido
	call escribe_en_vram		;5bee
	inc hl			;5bf1
	ret			;5bf2
apaga_un_hueco_de_la_cola:
	ld c,(hl)			;5bf3   ; el tipo de pieza
	ld (hl),000h		;5bf4   ; el hueco queda libre
	inc hl			;5bf6
	ld d,(hl)			;5bf7
	inc hl			;5bf8
	ld e,(hl)			;5bf9
	inc hl			;5bfa
	ld b,(hl)			;5bfb
	call posicion_a_celda		;5bfc   ; la posicion, en celda
	ld hl,05c14h		;5bff   ; los diez ceros: borra por donde paso
	push de			;5c02
	push bc			;5c03
	call pinta_pieza_de_dos_filas		;5c04
	pop bc			;5c07
	pop de			;5c08
	ld a,020h		;5c09
	call suma_a_a_de		;5c0b
	ld a,c			;5c0e
	add a,00fh		;5c0f   ; quince mas: la fila de abajo
	ex af,af'			;5c11
	jr pinta_dos_o_tres_celdas		;5c12
...
; ----------------------------------------------------------------------
; LA COLA CONTRA EL JUGADOR. Solo se mira en los estados por debajo de 5, y la ventana no es fija: el alto sale del propio tipo de pieza (`add a,a` tres veces mas ocho), o sea que una pieza mas alta se toca antes.
; ----------------------------------------------------------------------
mira_la_cola_contra_el_jugador:
	ld a,(0e134h)		;5c1e   ; el estado del jugador
	cp 005h		;5c21   ; de 5 en adelante, ya no
	ret nc			;5c23
	ld hl,0e25ch		;5c24
	ld b,004h		;5c27   ; los cuatro huecos
L_5C29:
	push hl			;5c29   ; se guardan los dos
	push bc			;5c2a
	ld a,(hl)			;5c2b   ; el estado de la pieza
	or a			;5c2c   ; vacio
	jr z,pasa_a_la_pieza_siguiente		;5c2d   ; al siguiente
	ld de,(0e130h)		;5c2f   ; la posicion del jugador
	inc hl			;5c33
	inc hl			;5c34   ; dos bytes mas alla
	ld a,d			;5c35   ; la fila del jugador
	sub (hl)			;5c36   ; menos la de la pieza
	add a,010h		;5c37   ; dieciseis de margen
	ld c,a			;5c39   ; en C
	inc hl			;5c3a   ; el byte siguiente
	ld a,(hl)			;5c3b   ; el tipo de pieza
	add a,a			;5c3c   ; por ocho
	add a,a			;5c3d
	add a,a			;5c3e
	add a,008h		;5c3f   ; mas ocho: el alto de la caja
	cp c			;5c41   ; comparado con la distancia
	jr c,pasa_a_la_pieza_siguiente		;5c42   ; fuera: no choca
	dec hl			;5c44
	dec hl			;5c45   ; dos atras
	ld a,e			;5c46   ; la columna del jugador
	sub (hl)			;5c47   ; menos la de la pieza
	add a,01ch		;5c48   ; veintiocho en la otra
	cp 024h		;5c4a   ; treinta y seis de ventana
	jr nc,pasa_a_la_pieza_siguiente		;5c4c   ; fuera de la ventana
	cp 004h		;5c4e   ; si cae en los cuatro de arriba, se ha saltado por encima
	jr c,lo_ha_saltado		;5c50
	pop bc			;5c52   ; se recuperan
	pop hl			;5c53
	scf			;5c54   ; acarreo: hay choque
	call se_ha_caido		;5c55   ; y si no, se choca
	ret			;5c58   ; y se vuelve

; ----------------------------------------------------------------------
; SALTAR POR ENCIMA DE UNA PIEZA, y aqui esta el premio por ENCADENAR: (0xE272) cuenta los saltos seguidos, sube hasta un tope de CUATRO, y lo que se cobra es ese contador por 512. O sea 512, 1024, 1536 y 2048 puntos: saltar seguido vale el cuadruple.
; ----------------------------------------------------------------------
lo_ha_saltado:
	ld a,c			;5c59   ; la distancia en fila
	cp 006h		;5c5a   ; seis pixeles de holgura
	jr c,pasa_a_la_pieza_siguiente		;5c5c   ; por debajo, no lo salto
	pop bc			;5c5e   ; se recuperan
	pop hl			;5c5f
	ld hl,051e9h		;5c60   ; la rampa del salto
	call arranca_gesto_con_cero		;5c63
	ld a,001h		;5c66
	ld (0e134h),a		;5c68   ; estado 1: en el aire
	ld a,(0e272h)		;5c6b   ; los saltos encadenados
	inc a			;5c6e   ; uno mas
	cp 004h		;5c6f   ; el tope son cuatro
	jr c,L_5C75		;5c71
	ld a,004h		;5c73
L_5C75:
	ld (0e272h),a		;5c75
	ld e,000h		;5c78
	add a,a			;5c7a   ; por dos, en el byte alto: 512 por cada uno
	ld d,a			;5c7b
	call suma_puntos		;5c7c
	ld a,001h		;5c7f   ; y el sonido del acierto
	call pide_un_sonido		;5c81
	ret			;5c84
pasa_a_la_pieza_siguiente:
	pop bc			;5c85
	pop hl			;5c86
	inc hl			;5c87   ; cuatro bytes por pieza
	inc hl			;5c88
	inc hl			;5c89
	inc hl			;5c8a
	djnz L_5C29		;5c8b
...
; ----------------------------------------------------------------------
; EL RITMO DE LA FIGURA GRANDE. El nibble bajo de (0xE1E0) mas uno indexa la tabla de 0x5F61 -14, 12, 10, 8, 6 y 4-, y ese es el numero de cuadros que dura cada paso: cuanto mas alto el nibble, MENOS cuadros, o sea mas deprisa. Y en el estado 2 el jugador puede empujar ese ritmo un paso arriba o abajo con la direccion que pulse.
; ----------------------------------------------------------------------
avanza_la_animacion_grande:
	ld hl,0e1f2h		;5ce3
	ld (hl),000h		;5ce6   ; se baja la senal de paso cumplido
	ld a,(0e1e0h)		;5ce8   ; el nivel de ritmo
	and 00fh		;5ceb   ; el nibble bajo
	inc a			;5ced
	ld b,a			;5cee
	ld a,(0e134h)		;5cef   ; el estado del jugador
	cp 002h		;5cf2   ; solo en el estado 2 se puede empujar
	jr nz,L_5D0B		;5cf4
	ld a,(0e009h)		;5cf6   ; la direccion pulsada
	rra			;5cf9
	rra			;5cfa
	and 003h		;5cfb   ; dos bits
	jr z,L_5D0B		;5cfd
	ld hl,0e1c1h		;5cff
	and 001h		;5d02
	xor (hl)			;5d04   ; cruzado con el sentido: si coinciden, acelera
	jr nz,L_5D0A		;5d05
	inc b			;5d07   ; un paso mas rapido
	jr L_5D0B		;5d08
L_5D0A:
	dec b			;5d0a   ; o uno mas lento
L_5D0B:
	ld a,b			;5d0b
	ld hl,05f61h		;5d0c   ; la tabla de duraciones
	call suma_a_a_hl		;5d0f
	ld b,(hl)			;5d12   ; los cuadros que dura este paso
	ld hl,0e1eeh		;5d13
	inc (hl)			;5d16   ; el contador
	ld a,(hl)			;5d17
	cp b			;5d18   ; llego?
	ret c			;5d19
	xor a			;5d1a
	ld (hl),a			;5d1b
	dec a			;5d1c
	ld (0e1f2h),a		;5d1d   ; se levanta la senal de paso cumplido
	ld hl,0e1c0h		;5d20
	ld a,(hl)			;5d23
	ld b,a			;5d24
	add a,001h		;5d25   ; el paso, en BCD
	daa			;5d27
	ld (hl),a			;5d28
	bit 4,a		;5d29   ; el bit 4 dice en que media vuelta va
	ld a,001h		;5d2b
	jr nz,L_5D30		;5d2d
	xor a			;5d2f
L_5D30:
...
; ----------------------------------------------------------------------
; COLOCAR LA FIGURA GRANDE. Dos tablas a la vez: la de alturas (0x521D o 0x5227) y la de velocidades con signo (0x5F7B o 0x5F85), las dos indexadas con el nibble bajo del mismo paso. La eleccion la hace el bit 6 de (0xE1E0), y el bit 1 de (0xE1E2) cambia al par suave.
; ----------------------------------------------------------------------
coloca_la_figura_grande:
	ld a,(0e1cah)		;5d33
	ld c,a			;5d36
	ld de,05f7bh		;5d37   ; las velocidades con signo
	ld hl,0521dh		;5d3a   ; y las alturas
	ld a,(0e1e0h)		;5d3d
	bit 6,a		;5d40   ; el bit 6 elige el par
	jr z,L_5D4E		;5d42
	ld a,(0e1cbh)		;5d44
	ld c,a			;5d47
	ld a,(0e1e2h)		;5d48
	...
	jr z,L_5D72		;5d4c
L_5D4E:
	bit 1,a		;5d4e   ; el bit 1, la variante suave
	jr z,L_5D58		;5d50
	ld de,05f85h		;5d52   ; las velocidades suaves
	ld hl,05227h		;5d55   ; y sus alturas
L_5D58:
	ld a,b			;5d58
	push af			;5d59
	and 00fh		;5d5a   ; el nibble bajo: diez pasos
	push af			;5d5c
	call suma_a_a_hl		;5d5d   ; en la tabla de alturas
	pop af			;5d60
	call suma_a_a_de		;5d61   ; y en la de velocidades
	ld b,(hl)			;5d64
	ld a,(de)			;5d65
	ld hl,0e1d6h		;5d66
	ld (hl),b			;5d69   ; la altura, guardada
	pop de			;5d6a
	bit 4,d		;5d6b   ; el bit 4: la otra media vuelta
	jr z,L_5D71		;5d6d
	neg		;5d6f   ; el signo cambia en la otra media vuelta
L_5D71:
	add a,c			;5d71
L_5D72:
	ld (0e1d7h),a		;5d72   ; y la coordenada queda guardada
	ret			;5d75

; ----------------------------------------------------------------------
; MOVER LA FIGURA GRANDE. Coge a la vez su curva de alturas (0x5F67 o 0x5F71) y su tabla de velocidades con signo (0x5F7B o 0x5F85), las indexa con el nibble bajo del paso y le da la vuelta al signo de las dos segun el bit 4: con una sola mitad de la tabla se recorre la vuelta entera.
; ----------------------------------------------------------------------
mueve_la_figura_grande:
	ld a,(0e134h)		;5d76   ; el estado del jugador
	cp 004h		;5d79   ; el estado 4 tiene su propio camino
	ld a,(0e1f2h)		;5d7b   ; la senal de paso cumplido
	jr z,estado_4_con_el_decorado		;5d7e
	or a			;5d80
	ret z			;5d81   ; sin paso cumplido no hay movimiento
	ld a,(0e134h)		;5d82
	cp 003h		;5d85   ; el estado 3, tambien aparte
	jr z,estado_3_arrastrado		;5d87
	ld hl,05f67h		;5d89   ; la curva de alturas
	ld de,05f7bh		;5d8c   ; y las velocidades
	ld a,(0e1e0h)		;5d8f
	bit 1,a		;5d92   ; el bit 1 elige el par suave
	jr z,L_5D9C		;5d94
	ld hl,05f71h		;5d96
	ld de,05f85h		;5d99
L_5D9C:
	ld a,(0e1c0h)		;5d9c
	ld b,a			;5d9f
	and 00fh		;5da0   ; el nibble bajo: diez pasos
	push af			;5da2
	call suma_a_a_hl		;5da3
	pop af			;5da6
	call suma_a_a_de		;5da7
	ld a,(hl)			;5daa
	bit 4,b		;5dab   ; el bit 4: la otra media vuelta
	jr nz,L_5DB1		;5dad
	neg		;5daf   ; y el signo al reves
L_5DB1:
	push af			;5db1
	ld a,(de)			;5db2
	bit 4,b		;5db3
	...
	neg		;5db7
L_5DB9:
	add a,03dh		;5db9   ; mas 0x3D, que es el centro
	ld (0e1cah),a		;5dbb
	pop af			;5dbe
L_5DBF:
	push af			;5dbf
	ld hl,0e14ah		;5dc0
	add a,(hl)			;5dc3   ; se aplica a la posicion
	ld (hl),a			;5dc4
	call pinta_la_figura_grande		;5dc5   ; y se repinta
	pop af			;5dc8
	ld c,a			;5dc9
	bit 7,a		;5dca   ; el signo, para el desplazamiento del decorado
	ld b,000h		;5dcc
	jr z,L_5DD2		;5dce
	ld b,0ffh		;5dd0   ; byte alto a 0xFF: negativo
L_5DD2:
	call corre_el_decorado_bc		;5dd2
	jr borra_la_franja_de_abajo		;5dd5
estado_3_arrastrado:
	ld a,(0e138h)		;5dd7   ; la lectura congelada del mando
	push af			;5dda
	push af			;5ddb
	ld hl,0e1d4h		;5ddc
	add a,(hl)			;5ddf   ; se le suma a una coordenada
	ld (hl),a			;5de0
	pop af			;5de1
	ld hl,0e1cah		;5de2
	add a,(hl)			;5de5
	ld (hl),a			;5de6   ; y a la otra
	pop af			;5de7
	jr L_5DBF		;5de8
estado_4_con_el_decorado:
	push af			;5dea
	call cuenta_el_paso		;5deb   ; cuenta el paso
	call pinta_el_borde_del_decorado		;5dee   ; pinta lo que entra por el borde
	call desplaza_el_decorado		;5df1   ; y corre el decorado
	pop af			;5df4
	or a			;5df5
	ret z			;5df6
borra_la_franja_de_abajo:
	ld de,03880h		;5df7   ; la franja de abajo de la pantalla
...
; ----------------------------------------------------------------------
; PINTAR LAS DOS FIGURAS GRANDES. Cada una tiene su repertorio de poses -0x76EC y 0x7806 para la primera, 0x7876 y 0x797E para la segunda- y el bit 1 de (0xE1E0) elige entre los dos de cada par. La segunda solo se pinta si su posicion pasa de 8 y no adelanta a la referencia del decorado.
; ----------------------------------------------------------------------
pinta_las_dos_figuras:
	ld hl,0e1c0h		;5e10   ; el paso de la primera
	ld a,(0e1e0h)		;5e13
	ld bc,076ech		;5e16   ; su repertorio
	bit 1,a		;5e19   ; el bit 1 elige el otro
	jr z,L_5E20		;5e1b
	ld bc,07806h		;5e1d
L_5E20:
	ld de,03889h		;5e20   ; y su sitio en la pantalla
	bit 7,a		;5e23   ; el bit 7: hay segunda figura
	jr z,L_5E41		;5e25
	ld a,(0e1cah)		;5e27
	cp 008h		;5e2a   ; por debajo de ocho no se pinta
	jr c,L_5E44		;5e2c
	ld e,a			;5e2e
	ld d,020h		;5e2f
	call posicion_a_celda		;5e31
	ld a,(0e1e0h)		;5e34
	ld bc,07876h		;5e37   ; el repertorio de la segunda
	bit 1,a		;5e3a
	jr z,L_5E41		;5e3c
	ld bc,0797eh		;5e3e
L_5E41:
	call pinta_una_pose		;5e41
L_5E44:
	ld a,(0e1e2h)		;5e44   ; la segunda esta activa?
	or a			;5e47   ; a cero: no hay segunda
	ret z			;5e48
	ld a,(0e1e1h)		;5e49   ; su paso propio
	ld b,a			;5e4c   ; en B
	ld a,(0e1cah)		;5e4d   ; su posicion
	add a,b			;5e50   ; sumado
	ret c			;5e51   ; si desborda, se deja
	ld (0e1cbh),a		;5e52   ; guardada
	cp 008h		;5e55   ; ocho de margen
	ret c			;5e57   ; por debajo, se deja
	ld hl,0e14ch		;5e58   ; la referencia del decorado
	ld e,(hl)			;5e5b   ; leida
	cp e			;5e5c   ; y no puede adelantar a la referencia
	ret nc			;5e5d   ; y si la alcanza, se deja
	ld e,a			;5e5e   ; la posicion
	ld d,020h		;5e5f   ; y la fila fija
	call posicion_a_celda		;5e61   ; convertida en celda
	ld a,(0e1e2h)		;5e64   ; el estado otra vez
	ld bc,07876h		;5e67   ; una pareja de patrones
	bit 1,a		;5e6a   ; su bit 1
	jr z,L_5E71		;5e6c   ; sin el, la primera
	ld bc,0797eh		;5e6e   ; y con el, la otra
L_5E71:
	ld hl,0e1c9h		;5e71

; ----------------------------------------------------------------------
; PINTAR UNA POSE DEL REPERTORIO. Los cinco bits de abajo del paso dan la pose y, pasado el 15, se REFLEJA (`cpl / sub 5 / and 01fh`): las 32 posiciones se dibujan con once poses. Luego el guion se recorre con 0xFF de fin y 0xFE de fila nueva, y avanza o RETROCEDE segun si la pose es menor de seis: por eso la figura mira al otro lado sin datos aparte.
; ----------------------------------------------------------------------
pinta_una_pose:
	push bc			;5e74
	ld a,(hl)			;5e75
	and 01fh		;5e76   ; los cinco bits del paso
	cp 010h		;5e78   ; de la mitad en adelante
	jr c,L_5E81		;5e7a
	cpl			;5e7c   ; se refleja
	sub 005h		;5e7d
	and 01fh		;5e7f
L_5E81:
	ld c,a			;5e81
	pop hl			;5e82
	rlca			;5e83   ; la pose por dos: son punteros
	call suma_a_a_hl		;5e84
	push de			;5e87
	ld e,(hl)			;5e88   ; y de ahi el guion de la pose
	inc hl			;5e89
	ld d,(hl)			;5e8a
	ex de,hl			;5e8b
	pop de			;5e8c
L_5E8D:
	push de			;5e8d
L_5E8E:
	ld a,(hl)			;5e8e   ; el byte del guion
	inc hl			;5e8f
	ld b,a			;5e90
	inc b			;5e91   ; 0xFF: fin de la pose
	jr z,L_5EBA		;5e92
	inc b			;5e94   ; 0xFE: fila nueva
	jr nz,L_5E9F		;5e95
L_5E97:
	ld a,020h		;5e97   ; treinta y dos celdas
	pop de			;5e99
	call suma_a_a_de		;5e9a
	jr L_5E8D		;5e9d
L_5E9F:
	call escribe_en_vram		;5e9f   ; y al VDP
	ld a,c			;5ea2
	cp 006h		;5ea3   ; por debajo de seis se avanza, y por encima se retrocede
	jr c,L_5EAA		;5ea5
	dec de			;5ea7   ; hacia atras
	jr L_5EAB		;5ea8
L_5EAA:
	inc de			;5eaa   ; o hacia delante
L_5EAB:
	ld a,e			;5eab
	and 01fh		;5eac   ; final de fila
	jr nz,L_5E8E		;5eae
L_5EB0:
	ld a,(hl)			;5eb0
	inc hl			;5eb1
	inc a			;5eb2   ; 0xFF en el salto de fila: fin
	jr z,L_5EBA		;5eb3
	inc a			;5eb5   ; 0xFE: otra fila mas
	jr z,L_5E97		;5eb6
	jr L_5EB0		;5eb8
L_5EBA:
	pop de			;5eba
...
; ----------------------------------------------------------------------
; EL RITMO DE LA SEGUNDA FIGURA, con la misma tabla de duraciones que la primera (0x5F61) pero su propio contador y su propio paso en BCD. Asi las dos van a compases distintos con una sola tabla.
; ----------------------------------------------------------------------
avanza_la_segunda_figura:
	ld a,(0e1e2h)		;5ebc
	and 00fh		;5ebf   ; su nivel de ritmo
	inc a			;5ec1
	ld hl,05f61h		;5ec2   ; la misma tabla de duraciones
	call suma_a_a_hl		;5ec5
	ld b,(hl)			;5ec8   ; los cuadros que dura el paso
	ld hl,0e1c8h		;5ec9
	inc (hl)			;5ecc   ; su contador
	ld a,(hl)			;5ecd
	cp b			;5ece
	ret c			;5ecf
	xor a			;5ed0
	...
	inc hl			;5ed2
	ld a,(hl)			;5ed3
	add a,001h		;5ed4   ; y su paso, en BCD
	daa			;5ed6
	ld (hl),a			;5ed7
	jp borra_la_franja_de_abajo		;5ed8

; ----------------------------------------------------------------------
; EL RELEVO ENTRE LAS DOS FIGURAS. Cuando el decorado pasa de 0x160 y la segunda esta a tiro -menos de 16 contando los ocho de holgura- la segunda PASA A SER la primera: se copian sus dos bytes de ritmo y su paso de un golpe con un `ld hl,(...)` y un `ld (...),hl`.
; ----------------------------------------------------------------------
pasa_el_relevo_a_la_segunda:
	ld a,(0e134h)		;5edb   ; el estado del jugador
	cp 002h		;5ede
	ret z			;5ee0   ; en el estado 2 no hay relevo
	ld hl,(0e055h)		;5ee1   ; la posicion del decorado
	ld bc,0ff60h		;5ee4   ; el umbral
	add hl,bc			;5ee7
	jr nc,apaga_la_segunda		;5ee8
	ld hl,0e1cah		;5eea
	ld a,(hl)			;5eed
	add a,008h		;5eee   ; ocho de holgura
	cp 010h		;5ef0   ; y dieciseis de ventana
	ret nc			;5ef2
	ld a,(0e1e1h)		;5ef3   ; su paso propio
	add a,(hl)			;5ef6
	ld (hl),a			;5ef7
	ld hl,(0e1e2h)		;5ef8   ; el ritmo de la segunda
	ld (0e1e0h),hl		;5efb   ; pasa a ser el de la primera, los dos bytes de un tiron
	ld a,(0e1c9h)		;5efe   ; y tambien su paso
	ld (0e1c0h),a		;5f01
	ld bc,0e1e2h		;5f04
	jp L_5483		;5f07
apaga_la_segunda:
	xor a			;5f0a
	ld (0e1e2h),a		;5f0b   ; la segunda se apaga
	ret			;5f0e
repinta_lo_que_sujeta:
	ld a,(0e134h)		;5f0f   ; el estado del jugador
	ld b,a			;5f12
	cp 002h		;5f13
	jr nz,repinta_desde_arriba		;5f15
	ld a,(0e1f2h)		;5f17   ; la senal de paso cumplido
	or a			;5f1a
	ret z			;5f1b
	ld a,b			;5f1c
	cp 003h		;5f1d
	jr z,repinta_desde_arriba		;5f1f
	ld hl,05f58h		;5f21   ; el bloque de las seis celdas
	call descomprime		;5f24
	ld a,(0e1e1h)		;5f27
	rra			;5f2a   ; la mitad del paso
	ld b,a			;5f2b
	ld a,(0e1cah)		;5f2c
	add a,b			;5f2f
	sub 018h		;5f30   ; menos 0x18
	ret c			;5f32   ; por debajo, no hay nada que pintar
	ld (0e1d4h),a		;5f33
L_5F36:
	push af			;5f36
	and 006h		;5f37   ; dos bits: cuatro variantes
	ld hl,0735ah		;5f39   ; la tabla de rectangulos de 3x7
	ld bc,00307h		;5f3c
	...
	pop af			;5f46
	ld e,a			;5f47
	ld d,0a0h		;5f48   ; la fila 0xA0
	jp pinta_rectangulo		;5f4a
repinta_desde_arriba:
	ld hl,05f58h		;5f4d   ; el mismo bloque de seis celdas
...
; ----------------------------------------------------------------------
; EL MONTAJE DEL DECORADO, que es lo primero de cada fase. Carga los patrones de sprites, los del marco, monta lo comun y luego DESPACHA por el tipo de fase con la tabla pegada de 0x5FEF, dejando apilado el remate de 0x699D.
; ----------------------------------------------------------------------
monta_el_decorado_de_la_fase:
	ld hl,061bah		;5fd6   ; los patrones de sprites
	call descomprime		;5fd9
	ld hl,05fcfh		;5fdc   ; y los del marco
	call descomprime		;5fdf
	call monta_la_escena_de_la_fase		;5fe2   ; lo comun a todas las fases
	ld hl,0699dh		;5fe5   ; el remate, apilado a mano
	push hl			;5fe8
	ld a,(0e052h)		;5fe9   ; el tipo de fase
	call despacha_por_tabla		;5fec   ; y su tabla de cinco, pegada detras
...
L_5FFF:
	...
	ld de,00d00h		;600b
	ld bc,00100h		;600e   ; 256 celdas
	ld a,060h		;6011   ; con el patron 0x60
...
; ----------------------------------------------------------------------
; LIMPIAR EL ESTADO DE LA FASE: 160 bytes a cero desde 0xE130, en dos pasadas de 80, salvando antes la referencia del decorado con el juego alternativo de registros para no perderla.
; ----------------------------------------------------------------------
limpia_el_estado_de_la_fase:
	ld hl,(0e14ah)		;604c   ; la referencia, a salvo en el juego alternativo
	exx			;604f
	ld hl,0e130h		;6050
	ld b,050h		;6053   ; ochenta bytes
	call pon_a_cero		;6055   ; y otros ochenta
	call pon_a_cero		;6058
	exx			;605b
	ld (0e14ah),hl		;605c   ; la referencia, de vuelta
	ld a,0c3h		;605f
	...
	ldir		;6072
	ld de,0e120h		;6074   ; y la segunda copia
	ld c,010h		;6077
	ldir		;6079
	call que_dibujo_toca		;607b   ; que dibujo toca en esta fase
	jr nz,L_6095		;607e
	ex de,hl			;6080
	ld hl,0e0e0h		;6081
	ld b,014h		;6084   ; veinte sprites
L_6086:
	ld a,(hl)			;6086
...
L_6095:
	ld de,0e0b0h		;6095
	ld c,014h		;6098   ; veinte bytes al bufer de verdad
	ldir		;609a
	ld de,0619dh		;609c   ; la lista de siete
	call reparte_la_lista_de_siete		;609f
	ld a,002h		;60a2
	ld (0e210h),a		;60a4   ; la casilla de 0xE210
	ld hl,0e14ch		;60a7
	dec (hl)			;60aa   ; una menos
	inc hl			;60ab
	ld (hl),0a0h		;60ac   ; la fila 0xA0
	ld a,(0e052h)		;60ae   ; el tipo de fase
	and 00fh		;60b1
	call despacha_por_tabla		;60b3   ; y su tabla de cinco, pegada detras
...
L_60C0:
	...
	call reparte_la_lista_de_siete		;60c3
	ld hl,0e150h		;60c6
	ld b,004h		;60c9   ; cuatro moviles
L_60CB:
	dec (hl)			;60cb
	inc hl			;60cc
	ld (hl),048h		;60cd   ; a la fila 0x48
	inc hl			;60cf
	djnz L_60CB		;60d0
	ld hl,0800fh		;60d2   ; el estado inicial de los moviles
	ld (0e160h),hl		;60d5
L_60D8:
	ld hl,0e130h		;60d8
	ld (hl),085h		;60db   ; el jugador, en su sitio
	inc hl			;60dd
	ld (hl),03ch		;60de
	inc hl			;60e0
	ld (hl),000h		;60e1
	ld a,(0e002h)		;60e3
	bit 6,a		;60e6   ; el bit 6
	ret z			;60e8
	ld a,089h		;60e9   ; y el sonido de arranque
	ex af,af'			;60eb
	ld a,006h		;60ec
	ex af,af'			;60ee
...
L_60F2:
	ld hl,0e1b0h		;60f2
	ld (hl),0d0h		;60f5   ; el suelto
	inc hl			;60f7
	ld (hl),050h		;60f8
	inc hl			;60fa
	ld (hl),050h		;60fb
	inc hl			;60fd
	ld (hl),00dh		;60fe   ; su plazo
	ld hl,059fah		;6100   ; y su rampa
	ld (0e1b6h),hl		;6103
	ld a,040h		;6106   ; el plazo del suelto
	ld (0e1bah),a		;6108
	ld (0e217h),a		;610b
	ld a,048h		;610e   ; la referencia del decorado
	ld (0e14dh),a		;6110
	ld a,03fh		;6113   ; y el jugador, en (0x3F, 0x3C)
pon_al_jugador_en_su_sitio:
	ld hl,0e130h		;6115   ; el jugador
	ld (hl),a			;6118
	inc hl			;6119
	ld (hl),03ch		;611a   ; la otra coordenada
	inc hl			;611c
	ld (hl),000h		;611d
	ld a,08bh		;611f   ; el sonido de esta fase
	ex af,af'			;6121
	ld a,006h		;6122
	ex af,af'			;6124
	call pide_un_sonido		;6125
baja_los_tres_recogibles:
	ld hl,0e170h		;6128   ; los tres primeros recogibles
	dec (hl)			;612b   ; uno menos cada uno
	inc hl			;612c
	inc hl			;612d
	dec (hl)			;612e
	inc hl			;612f
	inc hl			;6130
	dec (hl)			;6131
	ld hl,0a00fh		;6132   ; y su estado inicial
	ld (0e180h),hl		;6135
	ret			;6138
...
L_6142:
	...
	call reparte_la_lista_de_siete		;6145
	ld hl,05c8eh		;6148   ; el guion de la cola
	ld (0e26ch),hl		;614b   ; el puntero que avanza
	ld (0e25ah),hl		;614e   ; y el que recuerda el principio
	ld hl,0e26eh		;6151
	ld (hl),001h		;6154   ; la primera pieza, ya
	inc hl			;6156
	inc hl			;6157
	ld (hl),050h		;6158
	inc hl			;615a
...
L_6161:
	...
	ld (hl),02eh		;6164   ; la columna de partida
	inc hl			;6166   ; el byte siguiente
	ld (hl),03ch		;6167   ; la fila
	inc hl			;6169
	ld (hl),000h		;616a   ; el estado, a cero
	inc hl			;616c
	inc hl			;616d   ; dos mas alla
	ld (hl),002h		;616e   ; estado 2
	ld a,048h		;6170   ; el valor
	ld (0e14dh),a		;6172   ; la referencia del decorado
	ld a,001h		;6175   ; el paso
	ld (0e1c0h),a		;6177   ; el paso de la primera figura
	ld a,080h		;617a   ; la posicion
	ld (0e1cah),a		;617c   ; y la posicion de la segunda
	ld hl,0e1e0h		;617f   ; los cuatro bytes de ritmo
	ld (hl),041h		;6182   ; el primero
	inc hl			;6184
	ld (hl),090h		;6185   ; el segundo
	inc hl			;6187
	ld (hl),083h		;6188   ; y el tercero
	inc hl			;618a
	ld (hl),0a0h		;618b
	ld a,08dh		;618d   ; el sonido de esta fase
	ex af,af'			;618f
	ld a,004h		;6190
	ex af,af'			;6192
	jp pide_un_sonido		;6193
...
reparte_la_lista_de_siete:
	ld hl,0e0c7h		;61ab
	ld b,007h		;61ae

; ----------------------------------------------------------------------
; COPIAR B BYTES SALTEADOS, uno por cada cuatro del destino: es como se tocan solo los patrones de una tanda de sprites sin pisar sus filas ni sus colores.
; ----------------------------------------------------------------------
copia_uno_de_cada_cuatro:
	ld a,(de)			;61b0
	ld (hl),a			;61b1   ; el byte
	inc de			;61b2
	inc hl			;61b3   ; y cuatro bytes de salto: el siguiente sprite
	inc hl			;61b4
	inc hl			;61b5
	inc hl			;61b6
	djnz copia_uno_de_cada_cuatro		;61b7
	ret			;61b9
...
; ----------------------------------------------------------------------
; EL MONTAJE DE UNA ESCENA. Encadena TRES maquinas sobre el mismo HL sin volver a cargarlo: la escena de 0x6BE3 con 0x6A42, luego el bloque comprimido de 0x6D1A y luego mas listas de patrones. Cada una deja el puntero donde lo coge la siguiente.
; ----------------------------------------------------------------------
monta_la_escena_de_la_fase:
	ld hl,06be3h		;6968   ; la escena
	call monta_escena		;696b
	ld hl,06d1ah		;696e   ; el bloque que va detras
	ld de,04200h		;6971   ; su destino en la VRAM
	di			;6974
	call prepara_escritura_vram		;6975
	call vuelca_patrones_repetidos		;6978   ; primero las listas de patrones repetidos
	call descomprime_donde_quedo		;697b   ; y despues el descompresor, con el mismo HL
	call vuelca_lista_de_patrones		;697e
	ld de,00000h		;6981
	ld bc,00288h		;6984   ; 648 bytes
	call copia_a_los_tres_tercios		;6987
	call monta_la_franja_de_abajo		;698a
	call que_dibujo_toca		;698d
	ret nz			;6990
	ld de,00288h		;6991   ; y el resto, 1016
	ld bc,003f8h		;6994
	call copia_a_los_tres_tercios		;6997
	jp L_73B6		;699a
remata_el_montaje:
	call que_dibujo_toca		;699d   ; que dibujo toca en esta fase
	push af			;69a0
	call z,elige_el_decorado_alterno		;69a1
	pop af			;69a4
	ld hl,01000h		;69a5   ; los parametros de un decorado
	ld de,01800h		;69a8
	ld bc,02013h		;69ab
	jr nz,L_69B6		;69ae
	ld h,068h		;69b0   ; o los del otro, cambiando solo tres bytes
	ld d,070h		;69b2
	ld c,00ah		;69b4
L_69B6:
	ld (0e14ah),hl		;69b6   ; la referencia del decorado
	call pinta_bloque_en_su_sitio		;69b9
	ld hl,06d0ah		;69bc   ; y la escena que va encima
	call monta_escena		;69bf
	ld hl,0e14ah		;69c2
	jp pinta_la_figura_grande		;69c5
elige_el_decorado_alterno:
	ld hl,06ac1h		;69c8
	cp 002h		;69cb
	jr z,L_69D2		;69cd
	ld hl,06ac6h		;69cf
...
; ----------------------------------------------------------------------
; DOS BYTES DEL GUION que van a parar a B y C, y con ellos se revuelve el bufer: B por ocho es el desplazamiento dentro de el.
; ----------------------------------------------------------------------
lee_dos_bytes_del_guion:
	ld b,(hl)			;69d5   ; el primero
	inc hl			;69d6
	ld c,(hl)			;69d7   ; y el segundo
	inc hl			;69d8
	push hl			;69d9
	call prepara_la_figura		;69da   ; revuelve el bufer con ellos
	pop hl			;69dd
	ret			;69de

; ----------------------------------------------------------------------
; PINTAR EL BUFER EN LA VRAM, fila a fila. El primer byte es la pagina y cada fila trae su numero; un cero cierra. La direccion sale de multiplicar por ocho (`add hl,hl` tres veces) y ponerle el bit 6, que es la marca de escritura del VDP.
; ----------------------------------------------------------------------
pinta_el_bufer_por_filas:
	ld d,(hl)			;69df   ; la pagina del destino
	inc hl			;69e0
L_69E1:
	ld a,(hl)			;69e1   ; el numero de fila
	inc hl			;69e2
	or a			;69e3
	ret z			;69e4   ; un cero cierra la lista
	ld e,a			;69e5
	push de			;69e6
	ex de,hl			;69e7
	add hl,hl			;69e8   ; por ocho
	add hl,hl			;69e9
	add hl,hl			;69ea
	ex de,hl			;69eb
	set 6,d		;69ec   ; la marca de escritura
	di			;69ee
	call prepara_escritura_vram		;69ef
	ld de,0e280h		;69f2   ; el bufer de 0xE280
	call vuelca_por_mascaras		;69f5
	pop de			;69f8
	jr L_69E1		;69f9
...
pinta_el_bufer_con_ajuste:
	...
	or a			;69fd
	ret z			;69fe
	ld e,a			;69ff   ; el numero de fila
	ld d,005h		;6a00   ; la pagina, fija
	ex de,hl			;6a02
	add hl,hl			;6a03
	add hl,hl			;6a04
	add hl,hl			;6a05
	ex de,hl			;6a06
	set 6,d		;6a07   ; la marca de escritura
	di			;6a09
...
; ----------------------------------------------------------------------
; EL VOLCADO POR MASCARAS, que es lo mas bonito de este montaje: cada byte del guion es una MASCARA DE OCHO BITS y cada bit dice si el patron que toca del bufer se escribe o se salta. Con un byte se deciden ocho patrones, y por eso se puede repintar media pantalla sin tocar la otra media.
; ----------------------------------------------------------------------
vuelca_por_mascaras:
	ld c,(hl)			;6a1a   ; la mascara
	inc hl			;6a1b
	ld b,008h		;6a1c   ; ocho bits
	ex de,hl			;6a1e   ; HL pasa a ser el bufer y DE el guion
L_6A1F:
	rl c		;6a1f   ; el bit de arriba al acarreo
	push bc			;6a21
	ld bc,00008h		;6a22   ; ocho bytes, que es un patron
	call c,copia_literal		;6a25   ; bit a uno: se escribe
	add hl,bc			;6a28   ; y en los dos casos se avanza el bufer
	pop bc			;6a29
	djnz L_6A1F		;6a2a
	ex de,hl			;6a2c
	ld a,(hl)			;6a2d   ; otro byte de mascara, si no es cero
	or a			;6a2e
	jr nz,vuelca_por_mascaras		;6a2f
	inc hl			;6a31
	ret			;6a32

; ----------------------------------------------------------------------
; EL BUCLE DE MONTAJE, en sus dos variantes: descomprimir un trozo al bufer, leer sus dos bytes y volcarlo. Se repite mientras el byte siguiente no sea cero. La diferencia entre las dos puertas es solo con que rutina se pinta -0x69FB lee un ajuste por fila y 0x69DF no-, y por eso una escena medida con la puerta equivocada se pasa de largo.
; ----------------------------------------------------------------------
monta_escena_con_ajuste:
	call descomprime_a_ram		;6a33
	call lee_dos_bytes_del_guion		;6a36
	call pinta_el_bufer_con_ajuste		;6a39
	ld a,(hl)			;6a3c
	or a			;6a3d
	jr nz,monta_escena_con_ajuste		;6a3e   ; otro trozo mas
	inc hl			;6a40   ; y el cero que cierra
	ret			;6a41
monta_escena:
	call descomprime_a_ram		;6a42
	call lee_dos_bytes_del_guion		;6a45
	call pinta_el_bufer_por_filas		;6a48
	ld a,(hl)			;6a4b
	or a			;6a4c
	jr nz,monta_escena		;6a4d   ; otro trozo mas
	inc hl			;6a4f
	ret			;6a50

; ----------------------------------------------------------------------
; EL SEGUNDO DESCOMPRESOR, con el mismo lenguaje que el de la VRAM pero descargando en el bufer de 0xE280: bit 7 a cero repite un byte, bit 7 puesto copia n tal cual, y un 0x00 cierra.
; ----------------------------------------------------------------------
descomprime_a_ram:
	ld de,0e280h		;6a51   ; el bufer
L_6A54:
	ld a,(hl)			;6a54   ; el byte de control
	inc hl			;6a55
	or a			;6a56
	ret z			;6a57   ; cero: se acabo
	jp m,L_6A64		;6a58   ; bit 7 puesto: copia literal
	ld b,a			;6a5b   ; la cuenta
	ld a,(hl)			;6a5c   ; y el byte que se repite
	inc hl			;6a5d
L_6A5E:
	ld (de),a			;6a5e
	inc de			;6a5f
	...
	jr L_6A54		;6a62
L_6A64:
	and 07fh		;6a64   ; la cuenta, siete bits
	ld c,a			;6a66
	ld b,000h		;6a67
	ldir		;6a69   ; y copia de un tiron
	jr L_6A54		;6a6b
pinta_bloque_en_su_sitio:
	ld h,d			;6a6d
	call posicion_a_celda		;6a6e   ; de posicion a celda
	call que_dibujo_toca		;6a71   ; que dibujo toca
	ld a,h			;6a74
	ld hl,06aa9h		;6a75   ; una tabla
	jr nz,L_6A7D		;6a78
	ld hl,06a9eh		;6a7a   ; o la otra
L_6A7D:
	rra			;6a7d   ; tres giros: el indice
	rra			;6a7e
	rra			;6a7f
	and 01fh		;6a80
	call suma_a_a_hl		;6a82
	ld a,e			;6a85
	or 0e0h		;6a86   ; lo que queda hasta el borde derecho
	neg		;6a88
	cp b			;6a8a
	jr nc,L_6A8E		;6a8b
	ld b,a			;6a8d
...
saca_una_fila_del_bufer:
	ld a,(hl)			;6a95   ; un byte del bufer
	exx			;6a96
	out (c),a		;6a97   ; al puerto de datos
	exx			;6a99
	djnz saca_una_fila_del_bufer		;6a9a
	inc hl			;6a9c
	ld a,020h		;6a9d   ; y la fila de abajo, 32 celdas
...
monta_el_decorado_del_leon:
	...
	call vuelca_la_tanda_cuatro_veces		;6da3   ; la primera tanda, cuatro veces
	ld hl,06fc1h		;6da6
	call vuelca_la_tanda_cuatro_veces		;6da9   ; y la segunda, otras cuatro
	ld de,00b00h		;6dac
	ld bc,00280h		;6daf
	jp copia_dos_tercios		;6db2
vuelca_la_tanda_cuatro_veces:
	ld b,004h		;6db5
repite_la_tanda_cuatro_veces:
	push bc			;6db7   ; se guarda el puntero
	push hl			;6db8
	call vuelca_lista_de_patrones		;6db9   ; y se vuelca la misma tanda otra vez
	pop hl			;6dbc
	pop bc			;6dbd
	djnz repite_la_tanda_cuatro_veces		;6dbe
	ret			;6dc0
...
monta_el_decorado_de_las_bolas:
	...
	ld hl,0728fh		;71e5   ; y el bloque comprimido que la remata
	jp descomprime		;71e8
...
monta_la_franja_de_abajo:
	ld hl,072edh		;72e1
	call monta_escena		;72e4
	ld de,01680h		;72e7
...
monta_el_decorado_del_caballo:
	...
	ld de,02ac0h		;7483
	ld hl,02bb0h		;7486   ; el trozo que ya esta puesto
	ld bc,002d0h		;7489   ; 720 bytes de VRAM a VRAM
	call copia_vram_a_vram		;748c
	ld bc,003c0h		;748f
	ld de,00ac0h		;7492
	jp copia_dos_tercios		;7495
...
monta_el_decorado_del_trampolin:
	...
	ld hl,07598h		;7544   ; la tira de indices
	ld de,06680h		;7547   ; a un sitio
	call vuelca_con_espera		;754a
	ld de,065c0h		;754d   ; y al otro
	call vuelca_con_espera		;7550
	ld a,0f0h		;7553
	ld bc,00480h		;7555   ; 1152 celdas de relleno
	ld de,00380h		;7558
	call rellena_vram		;755b
	ld bc,00180h		;755e
	ld de,00500h		;7561
	jp copia_dos_tercios		;7564

; ----------------------------------------------------------------------
; VOLCAR UNA TIRA A LA VRAM CON ESPERAS. Es el mismo lenguaje del descompresor pero escribiendo directo al puerto, y los `push hl / pop hl` de 0x7578 no hacen nada: estan para dar tiempo al VDP entre byte y byte.
; ----------------------------------------------------------------------
vuelca_con_espera:
	di			;7567
	call prepara_escritura_vram		;7568
L_756B:
	ld a,(hl)			;756b   ; el byte de control
	inc hl			;756c
	or a			;756d   ; un cero cierra
	ret z			;756e
	ld b,a			;756f
	jp m,L_7580		;7570
	call recorta_el_indice		;7573
L_7576:
	out (098h),a		;7576   ; y al puerto
	push hl			;7578
	pop hl			;7579
	push hl			;757a
	pop hl			;757b
...
; ----------------------------------------------------------------------
; LA MARCA 0xFE DE LA PARTITURA: detras viene cuantas vueltas hay que dar. (ix+9) lleva las que van; al llegar a la cuenta la voz se calla, y si el byte es negativo el bucle no tiene fin. Es lo que hace que la musica de fondo se repita sola y un efecto suene una vez.
; ----------------------------------------------------------------------
repite_o_encadena:
	inc hl			;79fa
	ld a,(ix+009h)		;79fb   ; las vueltas dadas
	inc a			;79fe   ; una mas
	cp (hl)			;79ff   ; contra las que pide la partitura
	jp z,calla_la_voz		;7a00   ; se acabo: la voz calla
	jp m,L_7A07		;7a03   ; cuenta negativa: no se agota nunca
	dec a			;7a06
L_7A07:
	ex af,af'			;7a07
	ld a,(ix+002h)		;7a08
	push bc			;7a0b
	ld d,001h		;7a0c
	call reparte_el_sonido		;7a0e   ; y vuelve a arrancar el mismo sonido
	pop bc			;7a11
	ex af,af'			;7a12
	ld (ix+009h),a		;7a13   ; con las vueltas apuntadas
	ret			;7a16

; ----------------------------------------------------------------------
; ABRIR O CERRAR UN CANAL en el registro 7 del PSG. El bit que toca sale de rotar el numero de canal tres veces, y D dice si se pone o se quita. Ademas fuerza el bit 2 -el tercer canal de tono- salvo si el bit 5 esta puesto, en cuyo caso lo quita: ese canal es el que se cede al RUIDO.
; ----------------------------------------------------------------------
abre_o_cierra_el_canal:
	ld a,(0e031h)		;7a17   ; la copia del registro 7
	ld e,a			;7a1a
	ld a,c			;7a1b   ; el canal
	cp 001h		;7a1c
	jr z,L_7A21		;7a1e
	dec a			;7a20   ; el primero no lleva ajuste
L_7A21:
	rlca			;7a21   ; tres rotaciones: el bit del canal
	rlca			;7a22
	rlca			;7a23
	dec d			;7a24   ; D=1 pone, D=0 quita
	jr z,L_7A2B		;7a25
	cpl			;7a27   ; invertido y cruzado: se quita
	and e			;7a28
	jr L_7A2C		;7a29
L_7A2B:
	or e			;7a2b   ; o se anade
L_7A2C:
	set 2,a		;7a2c   ; el tercer canal de tono, abierto
	bit 5,a		;7a2e   ; salvo si esta pedido como ruido
	jr z,escribe_el_mezclador_del_psg		;7a30
	res 2,a		;7a32

; ----------------------------------------------------------------------
; EL MEZCLADOR DEL PSG, que es el registro 7 y NO el volumen: sus tres bits de abajo abren o cierran el TONO de cada canal, los tres siguientes el RUIDO, y los dos de arriba dicen si cada puerto de proposito general es de entrada o de salida -el A es por donde se leen los mandos, asi que tiene que quedar como entrada-. El valor de arranque es 0xB8, que es justo eso: los tres tonos abiertos, los tres ruidos cerrados y el puerto A como entrada. La copia en (0xE031) es la que consulta el reproductor de sonido para abrir y cerrar canales sin releer el PSG.
; ----------------------------------------------------------------------
escribe_el_mezclador_del_psg:
	ld e,a			;7a34   ; el valor, apartado en E
	ld (0e031h),a		;7a35   ; y la copia se guarda en RAM
	ld a,007h		;7a38   ; registro 7 del PSG
	jp 00093h		;7a3a   ; BIOS WRTPSG - Writes data to PSG-register

; ----------------------------------------------------------------------
; EL REPRODUCTOR DE SONIDO, una vez por cuadro y lo primero que hace la interrupcion. Lleva TRES voces, cada una con once bytes de estado a partir de 0xE010, y las recorre con IX saltando de once en once mientras C va 1, 3, 5: son los registros de periodo del PSG, dos por canal.
; ----------------------------------------------------------------------
suena_un_cuadro:
	ld a,(0e031h)		;7a3d   ; la copia del mezclador, que se vuelve a volcar tal cual en cada cuadro
	call escribe_el_mezclador_del_psg		;7a40
	ld c,001h		;7a43   ; el primer canal del PSG
	ld ix,0e010h		;7a45   ; el estado de la primera voz
	exx			;7a49
	ld b,003h		;7a4a   ; tres voces
	ld de,0000bh		;7a4c   ; once bytes cada una
L_7A4F:
	exx			;7a4f
	ld a,(ix+002h)		;7a50   ; si la voz esta callada, no se toca
	or a			;7a53
	call nz,avanza_una_voz		;7a54
	inc c			;7a57   ; dos registros por canal
	inc c			;7a58
	exx			;7a59
	add ix,de		;7a5a   ; y once bytes hasta la voz siguiente
	djnz L_7A4F		;7a5c
	ret			;7a5e

; ----------------------------------------------------------------------
; AVANZAR UNA VOZ. (ix+0) es lo que le queda a la nota en curso; mientras no llegue a cero no se lee nada de la partitura. Cuando se agota, se lee el byte siguiente, y el valor 0xFE es la marca de repeticion.
; ----------------------------------------------------------------------
avanza_una_voz:
	bit 6,a		;7a5f   ; el bit 6 del estado
	ld d,001h		;7a61
	call z,abre_o_cierra_el_canal		;7a63
	ld a,(ix+002h)		;7a66
	or a			;7a69
	jp m,sostiene_la_nota		;7a6a
	dec (ix+000h)		;7a6d   ; lo que le queda a la nota
	ret nz			;7a70   ; todavia suena: nada que hacer
L_7A71:
	ld l,(ix+003h)		;7a71   ; por donde va la partitura
	ld h,(ix+004h)		;7a74
	ld a,(hl)			;7a77
	cp 0feh		;7a78   ; 0xFE: la marca de volver al principio
	jp z,repite_o_encadena		;7a7a
	jr nc,calla_la_voz		;7a7d
	bit 7,(ix+002h)		;7a7f
	jp nz,lee_los_modificadores		;7a83
	and 0f0h		;7a86   ; el nibble alto manda
	cp 020h		;7a88   ; 0x2n cambia el instrumento de la voz
	jr nz,L_7A93		;7a8a
	ld a,(hl)			;7a8c
	and 00fh		;7a8d
	ld (ix+001h),a		;7a8f   ; el nibble bajo es el numero
	inc hl			;7a92
L_7A93:
	ld a,(hl)			;7a93
	and 0f0h		;7a94
	cp 010h		;7a96   ; 0x1n toca el envolvente
	jr nz,L_7AAC		;7a98
	ld a,006h		;7a9a
	ld a,(hl)			;7a9c
	and 01fh		;7a9d   ; y los cinco bits de abajo, su forma
	ld e,a			;7a9f
	ld a,006h		;7aa0
	call 00093h		;7aa2   ; BIOS WRTPSG - Writes data to PSG-register
	ld d,000h		;7aa5
	call abre_o_cierra_el_canal		;7aa7
	inc hl			;7aaa
	ld a,(hl)			;7aab
L_7AAC:
	ld b,(ix+002h)		;7aac   ; el estado de la voz
	bit 6,b		;7aaf   ; el bit 6: la voz esta encadenada a otra
	jr z,L_7AC4		;7ab1
	ld a,c			;7ab3
	cp 005h		;7ab4   ; y solo en el canal 5
	ld a,(hl)			;7ab6
	jr nz,L_7AC4		;7ab7
	inc hl			;7ab9
	ld (ix+003h),l		;7aba   ; se guarda por donde va la partitura
	ld (ix+004h),h		;7abd
	call fija_la_duracion		;7ac0
	ret			;7ac3
L_7AC4:
	and 0f0h		;7ac4   ; el nibble alto es la DURACION
	ld b,a			;7ac6
	xor (hl)			;7ac7   ; y el bajo, cruzado, la nota
	ld d,a			;7ac8
	inc hl			;7ac9
	ld e,(hl)			;7aca   ; el byte siguiente completa
	inc hl			;7acb
	ld (ix+003h),l		;7acc   ; la partitura queda apuntada
	ld (ix+004h),h		;7acf
	ex de,hl			;7ad2
	call escribe_la_nota		;7ad3   ; y se convierte en periodo
	ld a,b			;7ad6
	rrca			;7ad7
	rrca			;7ad8
	rrca			;7ad9
...
fija_la_duracion:
	ld h,a			;7adb
	ld a,(ix+001h)		;7adc   ; la duracion que toca
	ld (ix+000h),a		;7adf   ; a la cuenta atras de la nota
	add a,003h		;7ae2   ; y tres mas al contador de apagado
	ld (ix+008h),a		;7ae4
	jr escribe_el_volumen		;7ae7
calla_la_voz:
	xor a			;7ae9   ; se apaga el envolvente
	ld (ix+009h),a		;7aea
	ld d,001h		;7aed
	call abre_o_cierra_el_canal		;7aef
	xor a			;7af2
	ld (ix+002h),a		;7af3   ; y el estado de la voz, a cero: se calla
	ld h,a			;7af6
	jr escribe_el_volumen		;7af7

; ----------------------------------------------------------------------
; EL SOSTENIDO. Baja a la vez la cuenta de la nota (ix+0) y la del apagado (ix+8); cuando las dos coinciden y quedan menos de tres, empieza a bajar el volumen de (ix+7). Es lo que hace que una nota larga se apague sola sin una tabla de envolvente.
; ----------------------------------------------------------------------
sostiene_la_nota:
	dec (ix+000h)		;7af9   ; la nota
	jp z,L_7A71		;7afc   ; se acabo: a por la siguiente
	dec (ix+008h)		;7aff   ; y el contador de apagado
	ld a,(ix+008h)		;7b02
	cp (ix+000h)		;7b05   ; cuando se juntan
	jr nz,L_7B0F		;7b08
	cp 003h		;7b0a   ; y quedan menos de tres
	jr c,L_7B12		;7b0c
	ret			;7b0e
L_7B0F:
	dec (ix+008h)		;7b0f
L_7B12:
	ld a,(ix+007h)		;7b12   ; el volumen de la voz
	dec a			;7b15   ; uno menos
	ret m			;7b16   ; si llego a cero, ya no baja mas
	ld (ix+007h),a		;7b17
	ld h,a			;7b1a
escribe_el_volumen:
	ld a,c			;7b1b
	rrca			;7b1c   ; el canal por dos
	add a,088h		;7b1d   ; +0x88: el registro de volumen del PSG de ese canal
	ld e,h			;7b1f
	jp 00093h		;7b20   ; BIOS WRTPSG - Writes data to PSG-register

; ----------------------------------------------------------------------
; LOS MODIFICADORES DE LA PARTITURA, leidos en cadena por el nibble alto: 0xDn cambia el paso del arpegio, 0xFn el vibrato y 0xEn el volumen de la voz. Cada uno se lleva su byte y deja el siguiente en A, de modo que pueden venir varios seguidos delante de una nota.
; ----------------------------------------------------------------------
lee_los_modificadores:
	and 0f0h		;7b23
	cp 0d0h		;7b25   ; 0xDn: el paso
	ld a,(hl)			;7b27
	jr nz,L_7B31		;7b28
	and 00fh		;7b2a
	ld (ix+00ah),a		;7b2c   ; guardado en (ix+10)
	inc hl			;7b2f
	ld a,(hl)			;7b30
L_7B31:
	cp 0f0h		;7b31   ; 0xFn
...
L_7B3C:
	...
	jr c,L_7B47		;7b3e
	and 00fh		;7b40
	ld (ix+005h),a		;7b42   ; a (ix+5)
	inc hl			;7b45
	ld a,(hl)			;7b46
L_7B47:
	and 00fh		;7b47   ; el nibble bajo, que es cuantas veces
	ld b,a			;7b49
	ld a,(ix+00ah)		;7b4a
	jr z,L_7B54		;7b4d
L_7B4F:
	add a,(ix+00ah)		;7b4f   ; y se suma el paso esas veces: de ahi sale la duracion
	djnz L_7B4F		;7b52
L_7B54:
	ld (ix+001h),a		;7b54   ; la duracion de la nota
	ld a,(hl)			;7b57
	inc hl			;7b58
	ld (ix+003h),l		;7b59   ; y por donde se queda la partitura
	ld (ix+004h),h		;7b5c
	and 0f0h		;7b5f   ; el nibble alto del byte siguiente
	rrca			;7b61
	rrca			;7b62
	rrca			;7b63
	rrca			;7b64
	...
	sub 00ch		;7b66
	ld (ix+007h),a		;7b68
	jr z,nota_al_psg		;7b6b
	ld a,(ix+006h)		;7b6d
	ld (ix+007h),a		;7b70

; ----------------------------------------------------------------------
; DE NOTA A PERIODO. El indice saca uno de los doce periodos de la escala y (ix+5) dice cuantas octavas hay que bajar: cada `add hl,hl` dobla el periodo, que es bajar una octava justa.
; ----------------------------------------------------------------------
nota_al_psg:
	call fija_la_duracion		;7b73
	ld a,b			;7b76
	ld hl,07b96h		;7b77   ; la escala de doce
	call suma_a_a_hl		;7b7a
	ld l,(hl)			;7b7d   ; el periodo de esa nota
	ld h,000h		;7b7e
	ld a,(ix+005h)		;7b80   ; las octavas a bajar
	or a			;7b83
	jr z,escribe_la_nota		;7b84
	ld b,a			;7b86
L_7B87:
	add hl,hl			;7b87   ; doblar el periodo es bajar una octava
	djnz L_7B87		;7b88
escribe_la_nota:
	ld a,c			;7b8a
	ld e,h			;7b8b
...
; ----------------------------------------------------------------------
; PEDIR UN SONIDO. El 0x11 es el unico que se trata aparte: encadena TRES seguidos -el propio, el 0x52 y el 0x53-, que es la fanfarria de fin de fase.
; ----------------------------------------------------------------------
pide_un_sonido:
	di			;7ba2
	ld d,000h		;7ba3
	cp 011h		;7ba5   ; el sonido especial
	jr nz,L_7BB3		;7ba7
	call reparte_el_sonido		;7ba9   ; el primero
	ld a,052h		;7bac
	call reparte_el_sonido		;7bae   ; el segundo
	ld a,053h		;7bb1   ; y el tercero
L_7BB3:
	call reparte_el_sonido		;7bb3
	ei			;7bb6
	ret			;7bb7

; ----------------------------------------------------------------------
; REPARTIR UN SONIDO ENTRE LAS VOCES. El numero decide a que grupo va: por debajo de 0x89 son efectos -y dentro de ellos, el 6 y los que pasan de 0x12 tienen su propio hueco-, y de 0x91 en adelante van a una voz mas. Cada grupo lleva su PRIORIDAD: si lo que suena tiene el numero mas alto, el nuevo no entra.
; ----------------------------------------------------------------------
reparte_el_sonido:
	ld c,a			;7bb8   ; el numero de sonido
	ld b,002h		;7bb9   ; dos voces
	ld hl,0e012h		;7bbb   ; la voz de los efectos
	cp 089h		;7bbe   ; por debajo de 0x89: efecto
	jr c,L_7BC9		;7bc0
	cp 091h		;7bc2   ; de 0x89 a 0x90
	jr c,L_7BDB		;7bc4
	inc b			;7bc6   ; y de 0x91 en adelante, una voz mas
	jr compara_prioridades		;7bc7
L_7BC9:
	and 03fh		;7bc9   ; seis bits
	dec b			;7bcb
	cp 006h		;7bcc   ; el sonido 6 tiene su hueco
	jr z,L_7BDB		;7bce
	cp 012h		;7bd0   ; y los que pasan de 0x12, otro
	jr c,compara_prioridades		;7bd2
	jr z,L_7BDB		;7bd4
	ld hl,0e028h		;7bd6
	jr compara_prioridades		;7bd9
L_7BDB:
	ld hl,0e01dh		;7bdb
compara_prioridades:
	dec d			;7bde   ; D dice si se fuerza
	push af			;7bdf
	jr z,L_7BEE		;7be0
	ld e,(hl)			;7be2
	ld a,e			;7be3
	and 03fh		;7be4   ; la prioridad de lo que esta sonando
	ld (hl),a			;7be6
	ld a,c			;7be7
	and 03fh		;7be8
	cp (hl)			;7bea   ; contra la del que llega
	ld (hl),e			;7beb
	jr c,$+45		;7bec   ; si el nuevo pierde, no entra
L_7BEE:
	and 03fh		;7bee
	add a,a			;7bf0   ; el numero por dos
	ld de,07c19h		;7bf1   ; y la tabla de veintitres punteros
	call suma_a_a_de		;7bf4
	dec hl			;7bf7
	dec hl			;7bf8

; ----------------------------------------------------------------------
; ARRANCAR UNA VOZ: le pone la cuenta a uno para que suene en el cuadro siguiente, guarda el numero de sonido y copia los dos bytes del puntero a la partitura. Los cinco bytes que se salta con `ld a,005h / add a,l` son el resto del estado, que se deja como estaba.
; ----------------------------------------------------------------------
arranca_la_voz:
	ld (hl),001h		;7bf9   ; la cuenta, a uno: suena ya
	inc hl			;7bfb   ; el byte siguiente
	ld (hl),001h		;7bfc   ; a uno tambien
	inc hl			;7bfe   ; el siguiente
	ld (hl),c			;7bff   ; el numero de sonido
	inc hl			;7c00   ; el siguiente
	ld a,(de)			;7c01   ; y su partitura, dos bytes
	ld (hl),a			;7c02   ; el byte bajo
	inc hl			;7c03
	inc de			;7c04   ; y el alto
	ld a,(de)			;7c05
	ld (hl),a			;7c06   ; guardado
	ld a,005h		;7c07   ; cinco bytes mas alla del estado
	add a,l			;7c09   ; sumados al puntero
	ld l,a			;7c0a   ; que asi queda en el sitio
	ld (hl),000h		;7c0b   ; las vueltas, a cero
	inc hl			;7c0d   ; el byte siguiente
	pop af			;7c0e   ; lo que se guardo
	jr z,L_7C14		;7c0f
	ex af,af'			;7c11
	ld (hl),a			;7c12   ; y se apunta
	ex af,af'			;7c13
L_7C14:
	push af			;7c14
	inc hl			;7c15
	inc de			;7c16
	djnz arranca_la_voz		;7c17
